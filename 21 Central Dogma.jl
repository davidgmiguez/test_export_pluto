...
# ╔═╡ f6ee5edc-768e-4c6c-93d0-75de9d406675
using ParameterizedFunctions, PlutoUI, Plots, DifferentialEquations

# ╔═╡ c1c79f01-350c-4158-8cdd-afc96b0ac087
html"<button onclick='present()'>present</button>"

# ╔═╡ 79ab4851-2f19-45bc-af56-bf97904575f0
begin
	struct Foldable{C}
	    title::String
	    content::C
...
The value of [D] is often a scalar that denotes the copies of the gene [D], which is often one or two, but in cancer cells, a copy of a gene can be multiplied due to errors in mitosis, resulting in gene duplication.  
##
...
##
To calculate it we use the boundaty condition of teh initial concetration of $M(0)$
```math
\begin{align*}
...
begin
	...
	M₀_slide = @bind M₀ html"<input type=range min=0.0 max=1.0 step=.1>"
	k_M_slide = @bind k_M html"<input type=range min=0.001 max=0.005 step=0.001>"

		md"""##
**Set the values of the parameters of the simulation**
...
begin
	γ_M_slide = @bind γ_M html"<input type=range min=0.001 max=0.05 step=.005>"
...
This simplified approximation does not take into acount that promoters are not perfect, and that there is often a basal production of mRNA that is not negligible. 
The next set of interactions includes the basal production of `M` in absence of transcription factor `T` with a rate $\alpha_0$:
...
```
##
Following the same steps as the previous equation, we obtain the following solution:
...
begin
	k_0_slide = @bind k_0 html"<input type=range min=0 max=0.01 step=.005>"
...
begin
	...
	plot!(t,t-> M₀*exp(-γ_M*t)+( k_M*D*T)/γ_M*(1-exp(-γ_M*t)),label="\\ k_{0} = 0",seriestype=:line,ylims = (0,1))
...
This first version of the model assumes that one molecule of transcription factor $T$ results in one molecule of `mRNA`. We known that this is not how transcription works. Normally, a molecule of transcription factor is able to recruit many molecules of RNA polymerase $pRNA$ and activate transcription of many molecules of $M$. 
##
Therefore, a more realistic way of working with differential equations to describe transcription is to substitute number of molecules of transciption factor [T] in the equation by a function where activation and deactivation of transcription of $M$ depends on the amount of [T]. 
##
For instance:
...
where $\Phi ([T])$ can be interpreted as the probability that a gene copy is transcribed at a given time, as a function of $[T]$. 
##
An additinal very important advantage of this approach is that it allows us to introduce the concept of a  transcription factor as activators (its presence results in activation of transcription) or as repressors (its presence results in repression of transcription). 
##
...
begin
	...
	plot(P3,P4,layout=(1,2),legend=true,size = (800, 400))
end
...
begin
	TT_slide = @bind TT html"<input type=range min=0.5 max=1.5 step=.1>"
...
begin
	P51=plot(t,t-> M₀*exp(-γ_M*t)+(k_0 *D *D + k_M*D*TT.^nn./(K^nn.+TT^nn))/γ_M*(1-exp(-γ_M*t)),label="\\ M with T= $TT",seriestype=:line,ylims = (0,1))
title!("Activator")
	xaxis!("Time")
	yaxis!("Concentration")
	
	P61=plot(t,t-> M₀*exp(-γ_M*t)+(k_0 *D *D + k_M*D*K^nn/(K^nn.+TT^nn))/γ_M*(1-exp(-γ_M*t)),label="\\ M with T= $TT",seriestype=:line,ylims = (0,1))
title!("Repressor")
	xaxis!("Time")
	yaxis!("Concentration")
	...
	plot(P51,P61,layout=(1,2),legend=true,size = (700, 300))
end

# ╔═╡ e6db9987-8540-4ad9-ba24-f8d46a15837b
md"We can see now that, when concentration of [T] is above a certain value, transcription is activated (for [T] as activator) or repressed (for [T] as repressors).  
##
Binding and unbinding of [T] to the promoter is a highly dynamic procces, and more realistic scenario is that the probability of transcription of a gene $\Phi ([T],[D])$ depends on the time the trascription factor [T] is bound to the DNA [D]. 
##
This binding and unbinding can be written as a chemical interaction as:
```math 
...
where $K_D$ is the chemical equilibrium constant. 
##
We have also an extra constraint given by the fact that the total number of DNA copies `[D]` is fixed, therefore:
...
4. The maximum slope  controlled by the parameter `n`, with is calles sigmoidicity. To study the value of the slope and its correlation with the value of `n`, we divide numerator and denominator by $K^n$ and define the variable x as $x= T/K$. 
##
This way the normalized version of the Hill function is: 
...
begin
	...
	P20=plot(T_vector,ϕ_a,yaxis=[0,1],lab="Hill Function activator")
	#title!("Hill Function")
	xlabel!("Concentration of T")
	ylabel!("fraction of active promoters")

	ϕ_r=K.^n./(K.^n.+T_vector.^n)
	P21=plot(T_vector,ϕ_r,yaxis=[0,1],lab="Hill Function repressor")
	
	#title!("Hill Function repressor")
	xlabel!("Concentration of T")
	ylabel!("fraction of active promoters")

		plot(P20,P21,layout=(1,2),legend=true,size = (800, 300))

end

# ╔═╡ 14cbcf0e-8df1-4a60-80cb-e724e4b11da5
md" The Hill functions as activator or repressor are a more realistic approach to introduce the effect of a transcription factor into the Central Dogma Model. 
##
This way, the dynamics of production of [M] for activator is like this:  

```math
\frac{\mathrm{d} [M]}{\mathrm{d} t}= k_0 \cdot [D]+ k_M \frac{[T^n][D]}{K^n+[T^n]}-\gamma_M[M]\\
```
```math
\frac{\mathrm{d} [M]}{\mathrm{d} t}=\frac{\mathrm{d} [M]}{\mathrm{d} t}= k_0 \cdot [D] + k_M \cdot [D] \frac{[T^n]}{K^n+[T^n]}-\gamma_M[M] \tag{79}
```
##
...
```math
\frac{\mathrm{d} [M]}{\mathrm{d} t}= k_0 \cdot [D] + k_M \frac{K^n[D]}{K^n+[T^n]}-\gamma_M[M]
```

```math
\frac{\mathrm{d} [M]}{\mathrm{d} t}= k_0 \cdot [D] + k_M \cdot [D] \frac{K^n}{K^n+[T^n]}-\gamma_M[M] \tag{80}
```
##
...
md" ## 
The analytical solution can be derived similarly as before, now with the production of mRNA modulated by teh Hill function. The resulting equations are:
...
For activators and for repressors, respectively.
"
...
begin
	...
	plot(P7,P8,layout=(1,2),legend=true,size = (800, 300))
	
end

# ╔═╡ 384c8d3f-c8a3-4f14-8355-9926446bb7cc
k_M

# ╔═╡ 94458f89-c342-44c9-bfd2-05e9352be704
n

# ╔═╡ 0dc65ff6-6b4c-4931-91b0-1cb7ba3c831d
γ_M

# ╔═╡ e0a3eaf0-8347-4131-acb3-f9b1fcb62e80
k_0

# ╔═╡ 709912b7-6519-41c1-91ac-89e907aa3e15
M₀

# ╔═╡ 3b359906-2082-48f4-ba76-256a3a092779
D

# ╔═╡ 5e4f8ec2-72e5-45d5-b8ae-2dc267760da8
T
...
The first equation is the same as before, not surprisingly, since the addition of P does not affetc the amount of M.
##
...
md"##
We can write for simplicity thsi equation interms of the rates of production and mRNA using $\alpha_0 = [D]\cdot k_0$ as the basal production rate of mRNA. Similarly we can write $\alpha_M = [D]\cdot k_M$ as the maximum production of mRNA. 
...
plot(P9,P10,layout=(1,2),legend=true,size = (800, 300))
...
```
 ##
 The final analytical solutions can be obtained following the same strategy:
...
begin
	...
	sol3 = solve(prob3)
	sol4 = solve(prob4)
	P55=plot(sol3,label=["mRNA" "Protein"],ylims = (0,1))
	title!("Central Dogma activator")
	xlabel!("Time [s]")
	ylabel!("Concentration [nM]")
	P66=plot(sol4,label=["mRNA" "Protein"],ylims = (0,1))
	title!("Central Dogma repressor")
	xlabel!("Time [s]")
	ylabel!("Concentration [nM]")
	plot(P55,P66,layout=(1,2),legend=true,size = (800, 500))
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "551f362f9a449de0d5922b949b7c479c79b6286d"
