...
# ╔═╡ 6d470e46-951b-426f-8ec3-e82174ce2d66
begin
	struct Foldable{C}
	    title::String
	    content::C
...
md"# 7. Solving Differential Equations of Systems of Interactions Analytically 
...
```
##
It is called a first-order reaction because the rate is proportional to the first power of the concentration of one reactant. Taking eq. 2, we reorganize variables and integrate them.
...
md" 
> __Task 1a__: Write a computer program that consists on a function that takes three inputs: (1) the time of the reaction, (2) a vector of initial concentrations of the species $X_i$, and (3) the reaction rate $k$. The function should return a matrix of two rows that correspond to the time evolution of the two variables in the irreversible first-order interaction, based on the following solution: 
>
>```math
>\begin{align*}
>X_1 (t) &= X_1 (0) \cdot e^{- k_1 t} \\
>X_2 (t) &=  X_2 (0) + X_1 (0) (1-e^{- k_1 t})  \tag{23}
>\end{align*}
>```
> __Task 1b__:With these values, generate a plot of the time evolution of the variables $X_i(t)$  first order reaction of k=0.5, $X_1(0) =0.1$, and $X_2(0) =0.2$ and t= 10 s. Plot also the conservation law obtained $X_1(t)+X_2(t)$. 
"
...
begin
	...
	X₀ = [0.1,0.2]
	k_1 = 0.5;
	p11=plot(t,t->X₀[1]*exp(-k_1*t),label="\\ X_1",seriestype=:line)
	p11=plot!(t,t->X₀[2]+X₀[1]*(1-exp(-k_1*t)),label="\\ X_2",seriestype=:line)
	p11=plot!(t,t->X₀[1]*exp(-k_1*t)+X₀[2]+X₀[1]*(1-exp(-k_1*t)),label="Conservation Law",seriestype=:line)
	title!("Irreversible First order reaction")
	xaxis!("Time")
	yaxis!("Concentration")
	nothing
end

# ╔═╡ 19aefae9-2f94-4a2d-9c4f-dc9aed48844a
Foldable("Click to reveal solution of Task 1):",p11) 
...
```
##
To solve this diferential equation, we need to calculate the integration factor, $e^{ \int p(x)dx }$, that allows us to put the first term of the equation as one diferentiation. In this case $p(x)=k_2+k1 $, so $e^{ \int (k_2+k1)dt }=e^{(k_2+k1)t}$. 
##
This way, when we multiply both terms in eq. 18 by the integrating factor, we obtain:
...
md" 
> __Task 2a__: Write a computer program that consists of a function that takes three inputs: (1) the time of the reaction, (2) a vector of initial concentrations of the species $X_i$, and (3) a vector with the two reaction rates $k_1$ and $k_2$. The function should return a matrix of two rows that correspond to the time evolution of the two variables in the irreversible first-order interaction, based on the following solution: 
>
>```math
>\begin{align}
>X_1(t)&=\frac{k_2\left ( X_1(0)+X_2(0) \right )}{k_1+k_2}+\frac{k_1 X_1(0)-k_2X_2(0) }{k_1+k_2}e^{-\left (k_1+k_2)\right )t} \\
>X_2(t)&=\frac{k_1 (X_1(0)+X_2(0))} {k_1+k_2}-\frac{k_1 X_1(0)-k_2X_2(0) }{k_1+k_2}e^{-\left (k_1+k_2)\right )t} 
>\end{align}
>```
> __Task 2b__:With these values, generate a plot of the time evolution of the variables $X_i(t)$  first order reaction of $k_1=0.5$ and $k_2=0.4$, $X_1(0) =0.1$, and $X_2(0) =0.2$ and t= 10 s. Plot also the conservation law obtained $X_1(t)+X_2(t)$. 
"
...
begin
	...
	t_final=10
	k_2 = 0.4
	X₀[1] = 0.1 #🔴 # Initial concentrations
	X₀[2] = 0.2 #🟡 # Initial concentrations
p22=plot(t,t->((k_2*(X₀[1]+X₀[2]))/(k_1+k_2)) + ((k_1*X₀[1]-k_2*X₀[2])/(k_1+k_2))*exp(-t*(k_1+k_2)),label="\\ X_1",seriestype=:line)
plot!(t,t->X₀[1] + X₀[2] - ((k_2*(X₀[1]+X₀[2]))/(k_1+k_2)) - ((k_1*X₀[1]-k_2*X₀[2])/(k_1+k_2))*exp(-t*(k_1+k_2)),yaxis=[0,0.3],label="\\ X_2",seriestype=:line)
...
# ╔═╡ 321fe21b-5ae2-4ea9-b65c-2891828097c7
Foldable("Click to reveal solution of Task 1):",p22) 

# ╔═╡ bea913a2-ccf1-44e5-b710-8d0c8568605f
md"## 3. Second order reaction: irreversible dimerization

A special case of second order reaction is when conversion into product occur in pairs. This corresponds to a situation known as dimerization, very common in biology. In the next section, we will derive the solution for this scheme of interaction, where two identical molecules interact to form a complex. 
##
The general scheme of interaction is the following:
...
```
##
so, multiplying the numerator and denominator of the right side of the equation by $X_1(0)$ we obtain
...
Let's now use the conservation of mass to obtain the equation for the reactant $X_2$. Remember, if $C\cdot(B-A)^T =0$, then $\sum_{i=1}^{r} C_i \cdot X_i = cte $, so
...
md"  
> __Task 3a__: Write a computer program that consists of a function that takes three inputs: (1) the time of the reaction, (2) a vector of initial concentrations of the species $X_i$, and (3) a vector with the reaction rate $k_1$. The function should return a matrix of two rows that correspond to the time evolution of the two variables in the irreversible first-order interaction, based on the following solution: 
>
>```math
>\begin{align}
>X_1(t) &= \frac{X_1(0)}{2k_1 X_1(0) \cdot t + 1}\tag{65}\\
>X_2(t) &= X_2(0) +  \frac{k_1 X_1(0)^2 t}{2k_1 X_1(0) \cdot t + 1}\tag{81}\\
>\end{align}
>```
> __Task 3b__: With these values, generate a plot of the time evolution of the variables $X_i(t)$  first order reaction of $k_1=0.5$ and $k_2=0.4$, $X_1(0) =0.1$, and $X_2(0) =0.2$ and t= 10 s. Plot also the conservation law obtained $X_1(t)+X_2(t)$. 
>
>  __Task 3c__: Compare the dynamics of the consumption of $X_1$ between this second-order reaction and the first-order irreversible reaction. Why are they so different, even when we use the same values?"
...
md"We cannot compare because the units of the kinetic constants are different between first-order [1/time] and second-order [1/(time*concentration)] reactions. To compare the dynamics, we should adjust these two values. One way of comparing them is to divide the kinetic constant of the second-order dimer reaction by the initial concentration of $X_1$ "

# ╔═╡ 08fbbe98-e7eb-4d02-ad29-267453509c8c
begin
	P1=plot(t,t->X₀[1]*exp(-k_1*t),label="monomer reaction \\  X_1",yaxis=[0, 0.2],seriestype=:line)
	P1=plot!(t,t->X₀[2]+X₀[1]*(1-exp(-k_1*t)),yaxis=[0, 0.3],label="monomer reaction \\ X_2",seriestype=:line)
	title!("Monomer reaction")
#	k_1 = 1.0 / X₀[1]
	P2=plot(t,t->X₀[1]/((2*k_1/ X₀[1]*t*X₀[1])+1),label="dimer reaction \\ X_1 ")
	
	P2=plot!(t,t->X₀[2]+(k_1/ X₀[1]*t*X₀[1]^2)/((2*k_1/ X₀[1]*t*X₀[1])+1),yaxis=[0, 0.3],label="dimer reaction \\ X_2 ")
	title!("Dimer reaction")
...
The ODE systems that result from systems of interactions get complicated very fast, even for simple systems. This way, analytical solutions can only be obtained for almost trivial systems. Therefore, to study ODEs systems that correspond to more realistic scenarios, we need a different type of approach, such as numerical integration. We will learn how to do this in the next classes. 
"
...
manifest_format = "2.0"
project_hash = "7ce84d02dc5a9f7a6d2485e4c4e03c1339259083"
