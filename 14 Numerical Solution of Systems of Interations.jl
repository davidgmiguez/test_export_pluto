...
# ╔═╡ 7e08ce30-4179-4fbe-a4d0-81fe15388a7e
Euler_url = 
...
One of the main problems of these types of systems of ODE's is that they become increasingly complex very fast. Consequently, the analytical solution of simple reaction schemes becomes impossible to obtain. 
##
For instance, let's try the interaction scheme of reversible dimerization (a second-order reaction forward and a first-order reaction backward). The corresponding reaction scheme is as follows:
...
```
##
We can solve the integral but we cannot express the value of $X_1$ directly as a function of $t$, so we have to solve it using other methods. 

## First steps for the numerical solution of ODE'S:



This notebook is an introductory class on solving differential equations using numerical simulations. It has been implemented using Julia running on Pluto Notebooks. We define our problem as an irreversible chemical reaction in the following form. 

```math 
a + b \rightarrow c \tag{1}
```

where `a,b,c` are chemical species, and the rate or _speed_ of the reaction is determined by the reaction rate constant `k`, which represents the probability that a collision between `a` and `b` successfully produces a molecule `c`.
##
Using both the __Mass Action Law__ and the __Mass Conservation Law__, we can derive the minimum set of differential equations that correspond to this reaction. The general formulation of the system of differential equations can be expressed as:

```math 
u' = f(u,p,t) \tag{2}
```

where `f` is a general vector function that depends on the parameters `p` of the system, the time variable `t`. For this particular reaction, we have:
...
##
We will write the variables $a$,$b$,$c$  in the form of a __state vector__, i.e., a column vector that contains all the variables of the system. In this particular problem:
...
The Euler method is the simplest most straightforward explicit algorithm to solve numerically a set of differential equations. Although it is quite simple, the Euler method often serves as the basis for constructing more complex methods to solve systems of equations numerically. 
##
Here, a differential equation can be thought of as a formula by which the slope of the tangent line to the curve can be computed at any point on the curve once the position of that point has been calculated. The idea is that while the curve is initially unknown, its starting point (i.e., it in the initial condition, which we denote by `u₀`) is known. Then, from the differential equation, the slope to the curve at `u₀` can be computed, and so, the tangent line.
$(Resource(Euler_url))
...
This last numerical solution corresponds to a situation where the numerical solution is estimated at every point. In other words, if we solve the ODEs for the first 100 seconds, it will solve the ODE's 100 times. 
##
This is ok for situations where the change is not high, so the approximation given by the slope is good enough. But in other cases, we will need more temporal resolution to obtain a valid solution.  In other cases, the change is so slow that we may not need to solve the equations as many times, resulting in a much faster simulation. 
 
##
The number of times you solve the system per time is defined by the __time-step__. In other words, the __time-step__ sets the number of iterations in the simulation (fast-changing dynamics will require more iterations than slow-changing dynamics).

To set the number of iterations, we define a new variable, 'timestep', and then define the matrix where values are stored and the tspan vector depending on the timestep selected."
...
md"
##
> __Task 1a__: Use the Euler method to solve numerically the system of reversible dimerization, defined by the system of equations:
>```math 
>\begin{align}
>\frac{dX_1}{dt} &= - 2k_1 \cdot X_1^2 + 2k_2 \cdot X_2 \tag{86} \\
>\frac{dX_2}{dt} &= k_1 \cdot X_1^2 - k_2 \cdot X_2 \tag{87} \\
>\end{align}
>```
>for the following parameter values: $k_1=0.05$, $k_2=0.005$, $X_1(0)=X_2(0)=1.0$ for 20 iterations and 100 seconds.  
>
>__Task 1b__: find the steady state and measure when the simulation reaches this value with a tolerance of 1%.
"
...
begin
	...
	W₀=[W₀1,W₀2];
	timestepZ=4
	tspanZ=LinRange(0, 100, floor(Int,100/timestepZ))
	W=zeros((size(W₀,1), size(tspanZ,1)));
	W[:,1]=W₀;
	for i = 2:size(tspanZ,1)
           W[1,i]= W[1,i-1]-k1*2*W[1,i-1]*W[1,i-1] + 2* k2* W[2,i-1] # Eq 1
           W[2,i]= W[2,i-1]+k1*W[1,i-1]*W[1,i-1] - k2* W[2,i-1] # Eq 2
       end

	p111=plot(tspanZ,W[1,:],label="a",seriestype=:scatter)
	plot!(tspanZ,W[2,:],label="b",seriestype=:scatter,yaxis=[0,2])
	title!("Euler in Julia")
	xlabel!("Time [s]")
	ylabel!("Concentration [M]")
	nothing
...
# ╔═╡ 7bccb451-6ba4-4388-ad81-ddd7424dc263
Foldable("Click to reveal solution of Task 1:",plot(p111))
...
manifest_format = "2.0"
project_hash = "ba0c1b876862ec5752fdfe01b6941b1001724283"
