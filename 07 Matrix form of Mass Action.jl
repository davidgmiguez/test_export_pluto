...
Previously, we have seen how any kind of system of interacting entities can be written using a chemical notation. Now, in order to solve these types of systems using computers, or when we deal with large numbers of species and or interactions, we will learn to write these systems using a a vector-matrix formulation. 
##
...
```
##
We start by writing the previous chemical reaction in a more general form, removing the name of each species by a variable $X_{i}$, where each different species is denoted with a different sub-index $i=1...s$ (where $s$ is the total number of species involved). 
 
```math
2 X_1 + X_2 \overset{k}{\longrightarrow}  2 X_3 \tag{34}
```

In this case,  $X_1$, $X_2$, and $X_3$ are the species $H_2$, $O_2$, and $H_2O$, respectively, and are the components of a vector  `X`, often called the state vector, because it represents the vector where we will store the state of each species. 
##
...
>
>```math
>Na_2CO_3 + CaCl_2 \overset{k_1}{\underset{k_2}{\longleftrightarrow}} CaCO_3 + 2 \cdot NaCl \tag{42}
>```
" 
...
>Generalize the function for reactions with an arbitrary number of reactants and reactions.
...
 md" 
##
> __Task 3__: (a) Use the function developed as part of task 2 to obtain the order of each reaction, the units of each of the rate constants, and the matrix (B-A)' for the following reaction 
>
>```math
>H_2+ I_2 \overset{k_1}{\underset{k_2}{\longleftrightarrow}} 2IH
>```
> (b) If $K_{eq}=0.40$ at a given temperature, find the equation that gives you the concentration of [$H_2$] at equilibrium based on the values of the $K_{eq}$ and the concentrations of equilibrium of the other reactants. (c) Write a function that uses as input the concentrations of equilibrium of the other reactants and provides as output the concentration of [$H_2$] at equilibrium. (d) Use the previous function to calculate the equilibrium concentration of [$H_2$] for [$HI$]=0.060 M and [$I_2$]=0.90 M, (e) If $E_a =1$ for forward reaction, calculate $E_a$ for the the backward reaction. (f) In thsi conditions, an increase in temperature will result in a shift towards more $H_2$ or more $HI$?
>
"
...
# ╔═╡ 8e8d2c3f-36ae-4e59-a49e-c2c067694ee8
Foldable("Click to reveal solution of e-f):",md"
- (e)
- (f). more HI because the foward reaction will benefit more from an increase in temperature. ")

# ╔═╡ b00cf633-b2db-4226-a790-66fd5f38b2be
md"  ## 
>__Exercise 4__: (a) Use the function developed as part of task 2 to obtain the order of each reaction, the units of each of the rate constants, and the matrix (B-A)' for the following reaction:
>
>```math
>N_2 + 3 H_2  \overset{k_1}{\underset{k_2}{\longleftrightarrow}} 2 NH_3
>```
>
> (b) Find the equation that gives you the concentration of [$H_2$] at equilibrium based on the values of the $K_{eq}$ and the concentrations of equilibrium of the other reactants. (c) Write a function that uses as input the $K_{eq}$ and the concentrations of equilibrium of the other reactants and provides as output the the concentration of [$H_2$] at equilibrium. (d) Use the previous function to estimate the concentration of [$H_2$] for $K_{eq}=13.7$, [$N_2$]= 1.88 M and [$NH_3$] = 6.62 M, (e) In which direction does the reaction shift (towards the left or the right in the reaction scheme) if: $H_2$ is added; $NH_3$ is added; $NH_3$ is removed."
...
manifest_format = "2.0"
project_hash = "e35736e900dba4c8f840d0f78120f5f472b5dfd4"
