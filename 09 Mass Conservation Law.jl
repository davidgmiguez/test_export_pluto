...
## 
It was first formulated by Antoine Lavoisier, considered by many as the father of modern chemistry, in the 18th century. 
...
simpleODE4! = @ode_def abetterway2 begin
  da = -k1 * a * b + k2 * c * d^2
  db = -k1 * a * b + k2 * c * d^2
  dc = k1 * a * b - k2 * c * d^2
...
```
##
so, we obtain one equation and three unknown variables. So the theorem says that for these relation between the values of the vector C, we can write the following:

```math
\begin{align}
(C_3 + 2 \cdot C_4 - C_2)  X_1 + C_2 X_2 + C_3 X_3 + C_4 X_4   = cte \tag{87}
\end{align}
```
...
```math
\begin{align}
C_3  X_1 + 2 \cdot C_4  X_1 - C_2  X_1 + C_2 X_2 + C_3 X_3 + C_4 X_4  = cte \tag{87}
\end{align}
```

If this is true for every value of $C_2$, $C_3$ and $C_4$, it will be true for each independent value of the three. Therefore, we can set two of them to zero, and obtain the value of the other. For instance, for $C_3=C_4=0$,

```math
\begin{align}
- C_2  X_1 + C_2 X_2 = cte \tag{87}
\end{align}
```

Again, if this is true, it will be true for each value of $C_2$, so we choose an arbitrary value of $C_2=$1, and we obtain:
...
\end{align}
```
this means that this relation between the two variables is true at all time points:

```math
\begin{align}
- X_1(t) + X_2(t) = cte \tag{87}
\end{align}
```
To see if this is correct, let's look and the numerical solution of the system and do the math $X_2(t)-X_1(t)$ for all time points:

"

# ╔═╡ 35faf42a-ad40-4587-b5e2-09decccacaba
begin
	
			prob2a = ODEProblem(simpleODE4!,[0.02,0.01,0.005,0.004],(0.0,300.0),(1,1.1))
		    sol2a = solve(prob2a)
			
			p11= plot(sol2a,title= "Full system",xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.03])
		
		p22=plot(sol2a.t, sol2a[2,:] .- sol2a[1,:],linestyle=:dot ,xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[-0.03,0.03],label=" X_2 - X_1",title="Conservation law 1")

	plot(p11,p22,layout=(1,2))
	
end

# ╔═╡ 10cb58f7-8597-4217-ba65-25533b6f2bdf
md"

So, the prediction is correct. Next, we can assume that if this correlation is true at all time points, it is also true at the initial time point. So, this can be used to find the constant value using the initial conditions (which are often known, since we start each process with a known number of units). 

```math
\begin{align}
- X_1(t) + X_2(t) = - X_1(0) + X_2(0)  \tag{87}
\end{align}
```
...
```math
\begin{align}
 X_2(t) =  X_2(0) + X_1(t) - X_1(0) \tag{87}
\end{align}
```

which makes sense, since the change in the number of molecules of $X_2$ should be the same as the change in $X_1$. We now can substitute this equation into the set of four differential equations, and remove the differential equation for $X_2$:

```math
\begin{align}        
            \frac{ \mathrm{d}X_1 }{\mathrm{d}t} &= - k_1 \cdot X_1 \cdot (X_2(0) + X_1 - X_1(0)) + k_2 \cdot X_3 \cdot X_4^2  \tag{83}\\ 
             \frac{ \mathrm{d} X_3 }{\mathrm{d}t} &=  k_1 \cdot X_1 \cdot (X_2(0) + X_1(t) - X_1(0)) - k_2 \cdot X_3 \cdot X_4^2  \tag{85}  \\
              \frac{ \mathrm{d} X_4 }{\mathrm{d}t} &= 2 k_1 \cdot X_1  \cdot (X_2(0) + X_1 - X_1(0)) - 2 k_2 \cdot X_3 \cdot X_4^2   \tag{86}  \\
\end{align}
```

now we do the same for the other variables, For instance, for $C_2=C_3=0$, and obtain $C_4$. This way the previous relationship:
...
```math
\begin{align}
2 \cdot C_4 X_1  + C_4 X_4  = cte \tag{90}
\end{align}
```
 taking an arbitrary value for $C_4$, for instance $C_4=1$, we obtain:
...
\end{align}
```
we see again if this is correct using the simulation:

"

# ╔═╡ 28173fb0-3fa6-4189-8e65-61f4510dcc39
begin
	
		
			p111= plot(sol2a,title= "Full system",xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.03])
		
		p222=plot(sol2a.t, 2 .* sol2a[1,:] .+ sol2a[4,:],linestyle=:dot ,xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.06],label=" 2 X_1 + X_4",title="Conservation law 2")

	plot(p111,p222,layout=(1,2))
	
end

# ╔═╡ 1528291f-fb86-438e-90f8-b1cec2c43683
md"
##
Again, since this is true at all time points, we obtain the link between the value of $X_1(t)$ and $X_4(t)$ via their initial concentrations:


```math
\begin{align}
2 X_1(t)  + X_4(t)  = 2 X_1(0)  + X_4(0) \tag{90}
\end{align}
```

now, we can use this relation to remove the differential equation of $X_4$, using:


```math
\begin{align}
X_4(t)  = X_4(0) + 2 (X_1(0) -  X_1(t))   \tag{90}
\end{align}
```

and substitute into the set of three differential equations, as:

```math
\begin{align}        
            \frac{ \mathrm{d}X_1 }{\mathrm{d}t} &= - k_1 \cdot X_1 \cdot (X_2(0) + X_1 - X_1(0)) + k_2 \cdot X_3 \cdot (X_4(0) + 2 (X_1(0) -  X_1))^2  \tag{83}\\ 
             \frac{ \mathrm{d} X_3 }{\mathrm{d}t} &=  k_1 \cdot X_1 \cdot (X_2(0) + X_1(t) - X_1(0)) - k_2 \cdot X_3 \cdot (X_4(0) + 2 (X_1(0) -  X_1))^2  \tag{85}  \\
\end{align}
```

Finally, we do the same for the other variables, For instance, for $C_2=C_4=0$, and obtain $C_3$:

```math
\begin{align}
C_3 X_1 + C_3 X_3  = cte \tag{87}
\end{align}
```
again, with $C_3=1$
...
\end{align}
```
Let's again test this:

"

# ╔═╡ e40f059c-d7bd-46e1-af06-1806927751b1
begin
	
		
			p1111= plot(sol2a,title= "Full system",xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.03])
		
		p2222=plot(sol2a.t, sol2a[1,:] .+ sol2a[3,:],linestyle=:dot ,xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.06],label=" X_1 + X_3",title="Conservation law 3")

	plot(p1111,p2222,layout=(1,2))
	
end

# ╔═╡ 0a96dc70-46d0-47a2-86f2-646c4aea9fcd
md"
...
```math
\begin{align}
 X_3(t) = X_1(0) + X_3(0) - X_1(t) +\tag{87}
\end{align}
```

that allows us to remove the extra equation, obtaining a system of one single differential equation:

```math
\begin{align}        
            \frac{ \mathrm{d}X_1 }{\mathrm{d}t} &= - k_1 \cdot X_1 \cdot (X_2(0) + X_1 - X_1(0)) \\ 
&+ k_2 \cdot (X_1(0) + X_3(0) - X_1) \cdot (X_4(0)
+ 2 (X_1(0) -  X_1))^2  \tag{83}\\ 
\end{align}
```
...
##
Let's test if the numerical solution of this simplified equation produces the same result as the full system of four differential equations: 
"

# ╔═╡ 9985f778-ce6b-4c2f-9dc1-6bfa8593a986
begin
	a₀_slide = @bind a₀ html"<input type=range min=0.02 max=0.06 step=0.01>"
    b₀_slide = @bind b₀ html"<input type=range min=0.015 max=0.04 step=0.01>"
	...
	k2_slide = @bind k2 html"<input type=range min=1.1 max=3.1 step=0.2>"
...
# ╔═╡ 6150dbf4-225d-40be-95cb-c27f9c7353d3
simpleODE5! = @ode_def abetterway3 begin
  da = -k1 * a * (b0 - aa0 + a) + k2 * (aa0 + c0 - a) * (d0 + 2 * ( aa0 - a))^2
    end k1 k2 aa0 b0 c0 d0 ;



# ╔═╡ 83d458e2-457b-41e2-a355-1d9fd4abb8d2
begin
	p = (k1,k2)
		...
		u₀=[a₀,b₀,c₀,d₀];
		tspan = (0.0,300.0)
		prob2 = ODEProblem(simpleODE4!,u₀,tspan,p)
	    sol2 = solve(prob2)
		
		sol2b = solve(prob2)
		p_1=plot(sol2b,title= "Full system",xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.1])

p_full = (k1,k2,a₀,b₀,c₀,d₀)
	u₀_full = [a₀]
	prob5 = ODEProblem(simpleODE5!,u₀_full,(0.0,300.0),p_full)
	    #sol2 = solve(prob2)
		nothing
	...
	p_2=plot(sol5,title= "Simplified System",xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.1],label=" a for simplified  system")	

	p_2=plot!(sol5.t,b₀ .- a₀ .+ sol5[1,:],linestyle=:dot ,xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.1],label=" b for simplified system")

	plot!(sol5.t,c₀ .+ a₀ .- sol5[1,:],linestyle=:dot ,xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.1],label=" c for simplified system")

	plot!(sol5.t,d₀ .+ 2 .* (a₀ .- sol5[1,:]),linestyle=:dot ,xlabel="Time [s]",ylabel="Concentration [M]",yaxis=[0,0.1],label=" d for simplified system")
...
manifest_format = "2.0"
project_hash = "8faf4194178e9240d4dad4307c9b6fefe42d06ac"
