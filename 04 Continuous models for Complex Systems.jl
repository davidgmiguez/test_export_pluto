...
# ╔═╡ a97e2aff-f2b4-4d17-bb45-f798e9a8e21a
md"#  4. Continuous Models 

Models for population growth based on discrete events have several  advantages (simplicity, easy mathematical analysis...), but they can be unrealistic (for, instance, they can produce chaotic dynamics, as seen in previous class). In addition, they assume that all events (cell division, cell death) ocurr in discrete time points, which it is far from what really happens in biological systems, wher events occur more like a contunuum in time."

# ╔═╡ 478eb095-1bc9-4757-afd0-998b9bee1d82
Resource("https://betterexplained.com/wp-content/webp-express/webp-images/uploads/2017/03/discrete-vs-continuous-growth.png")

# ╔═╡ 7ba042d6-107d-4188-a877-c7f7d7b35c46
md" 

# 4.1 Unconstrained population growth
...
md"
# 4.2 Constrained population growth: The Logistic model 
...
md" now if we want to know how long does it take for the population to double in size

```math
\begin{align*}
(K-N_0) e^{- \mu t} &=\frac{K N_0}{N} - N_0\\
(K-N_0) e^{- \mu t} &=N_0\frac{K - N}{N}\\
...
The doubling size means $N=2\cdot N_0$, so

```math
\begin{align*}
  \mu  &= \frac{1}{t(N=2 \cdot N_0)}Log(\frac{2 \cdot N_0 (K-N_0)}{N_0 (K - 2 \cdot N_0)})\\
  \mu  &= \frac{1}{t(N=2 \cdot N_0)}Log(\frac{2 (K-N_0)}{(K - 2 \cdot N_0)})\\
  \mu  &= \frac{1}{t(N=2 \cdot N_0)}(Log(2)+Log\frac{K-N_0}{K - 2 \cdot N_0})\\
\end{align*}
```

So, the growth rate is now related with the cell cycle and modulated by the distance between the carrying capacity and the number of cells. If we want to compare the changes in cell cycle with the value of teh uncostrained growth, we simply subsitute:

```math
\begin{align*}
  t(N=2 \cdot N_0)  &= \frac{1}{\mu}(Log(2)+Log\frac{K-N_0}{K - 2 \cdot N_0})\\
   t(N=2 \cdot N_0)  &= \frac{Log(2)}{\mu}(1+ \frac{Log\frac{K-N_0}{K - 2 \cdot N_0}}{Log(2)})\\
   t(N=2 \cdot N_0)  &= T (1+ \frac{Log\frac{K-N_0}{K - 2 \cdot N_0}}{Log(2)})\\
   t(N=2 \cdot N_0)  &= T (1+ Log_2\frac{K-N_0}{K - 2 \cdot N_0})\\
\end{align*}
```

So, the fact that the system is now constrained in growth adds an additional term to the cell cycle, so the average value increases as we approach half of the carrying capacity. 

"

# ╔═╡ 4bbadb0a-3aa4-4744-a623-1f2982425531
begin
	K_2_slide = @bind K_2 html"<input type=range min=100 max=200 step=10>"
	
	md"""
	**Set the carrying capacity**
	
	value of K: $(K_2_slide)
	
	"""
end

# ╔═╡ e1b9722e-a81c-4482-91ce-681d9982ed5f
K_2

# ╔═╡ a5c7161a-ce53-4a27-a238-21d66b67c57a
begin
	aaaa=collect(Float64,1:0.1:50)
	plot(aaaa, 1 .+ log2.((K_2 .- aaaa) ./ (K_2  .- 2 .* aaaa)),xlabel="N",ylabel="T'", label="K= $K_2",ylims=[0,10])
	plot!(aaaa, ones(491),xlabel="N",ylabel="t(N=2 N0)", label="unconstrained",ylims=[0,10])
end

# ╔═╡ eee0fd71-dc22-4fdd-9abd-5a0469e30872
md"The saturation at N=K/2 occurs because all cells are sincronized, so they divide at the same time. Therefore as soon as we approach half of the carrying capacity, there is no room for them to divide once more, so the cell cycle tends to infinite. "

# ╔═╡ a75c3669-c3a4-4989-b060-3a93e89867cb
md" It would be interesting to know what is the difference between simulations of syncronized versus de-sincronized populations in the constrained conditions. "
...
# ╔═╡ 9dacb5d8-1739-48a7-b8ea-e08593952ec0
begin
	N₀3_slide = @bind N₀3 html"<input type=range min=0 max=100 step=10>"
	K3_slide = @bind K3 html"<input type=range min=200 max=400 step=1>"

	md"""
	**Set the initial number of cells**
	
	value of N₀: $(N₀3_slide)
	value of K: $(K3_slide)
...
# ╔═╡ fdb9387a-352c-4392-9189-105781c1c7a2
Foldable("Solution Task 3:", md"##
...
# ╔═╡ 912f251e-159c-4e7a-98a5-9ff84067a2b5
begin
	plot(collect(0:1:100),logistic_growth_continuous(N₀/2,K,collect(0:1:100),TTT),label="T= $TTT",seriestype=:line,xlabel=("Time"),ylabel=("Number of cells"),ylims = (0,400))
...
begin
	plot(collect(0:1:150),logistic_growth_continuous(N₀3,K3,collect(0:1:150),TTT),label="T= $TTT",seriestype=:line,xlabel=("Time"),ylabel=("Number of cells"),ylims = (0,400))
	hline!([K3/2], label=" half carrying capacity")
	#vline!([TTT/log(2) * log((K/20)-1)], label=" time until half carrying capacity")
	vline!([TTT * log2((K3/N₀3)-1)], label=" time until half carrying capacity")
end
...
# ╔═╡ 6f007691-30ad-4667-84b4-f7760bbf8dd3
Resource("https://www.mittag-leffler.se/app/uploads/2023/06/shutterstock1925713271-1024x683-1.jpg")
...
##
Sometimes, in certain systems, the population benefits from a certain initial density that facilitates interaction or cooperation (increased probability to find a sexual partner, protection from the group, division of labor...)

##
In these systems, if the number of individuals decreases, they suffer from fewer or less efficient interactions and the per capita population growth rate decreases.
##
The critical population size below which the per capita population growth rate becomes negative is called the Allee threshold. 
##
A simple mathematical model of an Allee effect is one where initial densities below the Allee threshold lead to extinction, whereas initial densities above the Allee threshold lead to survival.
##

Another way of seeing this process is to focus on the effective growth
rate (dN/dt )/N. When this effective growth
rate is highest at intermediate N, we have a Allee effect (lower N means difficulty to find a partner, higher N means limit of resources, as in the logistic growth model). 

```math
\begin{align*}
\frac{1}{N}\frac{\partial N(t)}{\partial t}= max  
\end{align*}
```
##

For instance, lets consider this ODE

```math
\begin{align*}
\frac{\partial N(t)}{\partial t}= \mu N  - a N (b-N)^2  
\end{align*}
```
Teh first term is teh exponential growth as before, where $\mu = log2/(T)$
To show that this model has an allee effect, we just need to show that the effective growth rate has a maximum at intermediate values (not N=0, not N=infinite). To do that, we first need to find the fixed points, $\frac{\partial N(t)}{\partial t}=0$
...
has a solution at $N=0$ and $\mu-a(b-N)^2=0$. The second solution can be written as 

```math
\begin{align*}
(b-N)^2  = \frac{\mu}{a}\\
(b-N)  = \pm \sqrt{\frac{\mu}{a}}\\
N  = b \pm \sqrt{\frac{\mu}{a}}\\
\end{align*}
```

so, asumning that all constants are positive (also assuming that $b > \sqrt{\frac{\mu}{a}}$), we have three fixed points. To analyze the stability of the fixed points, we will plot the function of effective growth rate. 

"

# ╔═╡ 250cfc86-2cce-404c-9fc2-409b7a4bb26e
function Allee_effective_growth_continuous(N₀,a,b,T)
	μ=log(2)/T
    #N₀ .* exp.(μ .* t)
	μ .- a .* (N₀ .- b)^2
end;

# ╔═╡ 5a922b86-d962-4e3d-8a7a-bf324a202819
begin
	TTTT_slide = @bind TTTT html"<input type=range min=1 max=5 step=1>"
	a_slide = @bind a html"<input type=range min=1 max=5 step=0.5>"
	b_slide = @bind b html"<input type=range min=0.5 max=4 step=0.5>"

	
	md"""
	**Set the Cell cycle length and the Carrying Capacity**
	
	value of T: $(TTTT_slide) 
	value of a: $(a_slide)
	value of b: $(b_slide)
...
# ╔═╡ 8c44b80f-1652-4507-a80d-bfb1624345ed
begin
	plot(x ->  Allee_effective_growth_continuous(x,a,b,TTTT), collect(0:0.1:4),yaxis=([-1,2]),xaxis=([-0.5,4]),seriestype=:line,xlabel=("Number of individuals"),ylabel=("Effective growth rate [1/t]"))
	vline!([b + sqrt(log(2)/ a / TTTT)], label=" 3rd fixed point")
	vline!([b - sqrt(log(2)/ a / TTTT)], label=" 2nd fixed point")
	vline!([0], label=" 1st fixed point")
end

# ╔═╡ c1ee5438-6ef2-4bc6-a3b5-52d5c1c3bf94
md"So, we can see that we have an effective growth rate that is only positive maximum at intermediate values of N (the number of cells increases only at this regime). The location of this window is controlled by the $b$ parameter. The $a$ parameter regulates the size of the domain with positive growth rate, and the $T$  regulates the height (how fast or slow the system grows)"

# ╔═╡ 1c806c8e-34c8-412d-8682-3fbbebeb988f
md"

##

To analyze the stability of the fised points, we just need to focus on what happens to the points between the solutions. When the system is between first and second fixed points, the effective growth rate  is < 0, so they tend to reduce the $N$. Effectively, thsi means that the system moves away from point 2 and towards point 1.
##
The same happens for points above the third fixed point. The system evolves by reducing the number of individuals (towards point 3). 
##
Finally, situations where teh number of individulas is between point 2 and point 3, have a positive growth rate, so the number of individuals tends to increase. In other words, the system moves away from point 2 and towards point 3. 
## 
Based on this qualitative analysis, we can se that point 1 and point 3 are stable (teh system evolves towards them), while point 2 is unstable (the system evolves away from it). In the next classes we will learn how to use linear stability analysis to obtain this information and quantify this. 

"

# ╔═╡ cac3c0d3-64f8-4bb5-886c-5939452d9996
md"
##
>__Task 4:__ Perform a similar analysis of the following system:
>
>```math
>\begin{align*}
...
>\end{align*}
>```
>Calulate the fixed points and explain their stability based on the evolutin of the system in each regime. What is teh main difference with the previous Allee system?. 
 
"

# ╔═╡ c7cef332-0e50-48e0-bb6b-11964e0f201f
Foldable("Solution Task 4:", md"##

The strong Allee model, is analogous to the logistic growth model, except that the dependency on N in this model occurs in the opposite regime—introducing an Allee effect term that lowers the observed growth rate at small N near the Allee threshold A
```math
\begin{align*}
\frac{\partial N(t)}{\partial t}= \mu N(t) \left(1-\frac{A}{N(t)}\right) 
\end{align*}
...
This model describes N cells whose net growth rate exists in 2 distinct regimes: when N is less than the Allee threshold (A), the Allee effect term results in reduction in the population, while for population levels above the Allee threshold, the system is exponential. 
 ...
 Based on teh fixed points: We have one at $N=0$, and other at $N=A$. 

If we plot the effective growth rate and the fixed points (see below), we see that the firts point is stable, and the second is unstable (the systems grows exponentially away from it). 

")

# ╔═╡ 9c4c886e-d1f1-42db-aa5a-b09cd48ae7bd
function Allee_effective_growth_continuous2(N₀,A,T)
	μ=log(2)/T
    #N₀ .* exp.(μ .* t)
	μ .* (1 .- (A ./ N₀)) 
end;

# ╔═╡ cc39b065-aa7c-4bad-90f5-8236e7d0a82e
begin
	TTTTT_slide = @bind TTTTT html"<input type=range min=1 max=5 step=1>"
	A_slide = @bind A html"<input type=range min=1 max=5 step=0.5>"
...
# ╔═╡ 60b1d630-dd4e-4afe-98a5-efa7beda5216

Foldable("Solution for task 4:", plot(x ->  Allee_effective_growth_continuous2(x,A,TTTT), collect(0:0.1:4),yaxis=([-1,2]),xaxis=([-0.5,4]),seriestype=:line,xlabel=("Number of individuals"),ylabel=("Effective growth rate [1/t]")))
	


# ╔═╡ 45476d36-eddf-4492-9472-75f661d297c2
md"
##
>__Task 5:__ Perform a similar analysis of the following system:
>
>```math
>\begin{align*}
>\frac{\partial N(t)}{\partial t}= \mu N(t) \left(1-\frac{N(t)}{K}\right)\left(\frac{N(t)}{A} -1 \right) 
>\end{align*}
>```
> (a)Calulate the fixed points and explain their stability based on the evolutin of the system in each regime.
>
> (b) Write a possible differential equation for a population whose growth is modelled by a logistic equation with Allee effect if the intrinsic growth rate is 15%, the carrying capacity is 800 individuals, and at least 100 individuals are required in order for the population to survive.
...
manifest_format = "2.0"
project_hash = "ebcd8023f1624cfce9a94449b0989b118a4540ed"
