...
P(2T) = 2 \cdot P(T)
```
which is basically the same situation of unrestricted growth in bacterial populations, with a fixed $r=2$.
...
P(nT)= P_0(1+\gamma)^{nT}
```
So, in general and in comparison with the growth fraction parameter $r$ introduced when studying discrete bacterial populations, we can write:
...
begin
	time=collect(1:1:5);
	plot(time,2 .^(time),ylims = (0,10),label=("all cycling"),title=("Unconstrained growth with quiescence"),xlabel=("Generations [units of cell Cycle] "),ylabel=("Number of cells"))
...
md"## Discrete growth with quiescence
...
If we want to introduce this idea of quiescence via contact inhibition, we can use the same approach as the logistic growth, when division rate of the bacteria was reducing as we approach the carrying capacity of the system.

Now, cells do not stop because they run our of food, they stop dividing because they run out of available space. To introduce this in our model, we can start by making the growth fraction $\gamma$ dependent on the amount of cells:

```math
P(T)=P_0 \cdot (1+\gamma(P(T)))^{T} 
```

a first approach will be, equivalently to the logistic model, to define a linear dependence of the identity of cells as cycling or quiescence, depending on the nuber of cells in the system.

```math
\gamma(P(T)) = 1.0 - \frac{P(nT)}{K}
```

using this definition into the equation for growth we obtain:

```math
P(nT)=P_0 \cdot (1.0 + (1.0 - \frac{P(nT)}{K}))^{nT} 
```
...
which is again similar in form to the logistic solution, but now, instead of modulating the cell cycle from an initial value to a very long value, we are modulating the number of cells that are actively cycling, wich is more realistic.
...
growth_faction(P,K) = 2.0 - P/K;

# ╔═╡ 835cbcf6-f604-4172-8f71-f7e5496de990
begin
	K_slide = @bind K html"<input type=range min=10.0 max=100 step=10>"
...
begin
	#K=100
	p1=plot([0,K],[growth_faction(0,K),growth_faction(K,K)],xlims = (0,100),line = (:line, 4))
	title!("r for constrained growth")
	xlabel!("Number of cells")
	ylabel!("Growth Fraction")

	p2=plot([1:10],constrained_growth_generic(2,K; dt=1),ylims = (0,100),xlabel=("n (units of cell cycle)"),ylabel=("Number of cells"),title=("K=$K"))
	plot(p1,p2)
end

# ╔═╡ a362f508-3d2b-4ea8-8467-7c0dc880022a
md"  
## Continuous growth with quiescence

Since the resulting discrete equation is equivalent to the logistic growth. We can write a similar continuum approach, now in terms of the cell cycle $T$.
...
md"# 4.3 Robustness and the logistic growth
...
##
Here we will like to study if growing with limited resources increases or reduces the robustness. From the unconstrained growth:
...
```
##
now we assume a small change in the initial conditions:
...
So. in terms of cell cycle, we have a change in the time to reach a certain size of
...
# ╔═╡ ff488ced-be65-4ba3-93ec-38f6757689f9
begin
	plot(x -> log2(1+x), collect(0:0.1:1),seriestype=:line,xlabel=("amount of perturbation"),ylabel=("relative change in developmental time"),label=("Increase"))
	plot!(x -> log2(1-x), collect(0:0.1:1),seriestype=:line,label=("Decrease"))
end

# ╔═╡ 74a1d582-64c3-4a97-9c9c-66598155f5a9
md"This means it is not the same effect an increase of 50% in the population than a decrease of 50%. The decrease is allways more important, in terms of how it translates into the final numbers of cells. 
"

# ╔═╡ c023db26-e3fb-447e-a5a5-7d3ef9f84d3b
function unconstrained_growth_continuous(N₀,t,T)
	μ=log(2)/T
    N₀ .* exp.(μ .* t)
end;

# ╔═╡ d7dd1927-8c0d-4fe4-8904-b38e815213ef
begin
...
# ╔═╡ 0adcb201-9506-44ad-a961-ad87423fa26d
begin
	plot(collect(0:1:100),unconstrained_growth_continuous(20,collect(0:1:100),TT),label="No perturbation",seriestype=:line,xlabel=("Time"),ylabel=("Number of cells"),ylims = (0,400))
	
	plot!(collect(0:1:100),unconstrained_growth_continuous(20+10,collect(0:1:100),TT),label="Increase of 50% in N0",seriestype=:line,xlabel=("Time"),ylabel=("Number of cells"),ylims = (0,400))

	plot!(collect(0:1:100),unconstrained_growth_continuous(20-10,collect(0:1:100),TT),label="decrease of 50% in N0",seriestype=:line,xlabel=("Time"),ylabel=("Number of cells"),ylims = (0,400))
end

# ╔═╡ 449c6a01-2962-4c62-af22-e5ef050918b7
md" we can see how the difference in time to reach the value of 400 cells is higher when the perturbation is in the form of a decrease in the initial number of cells. Now for the logistic growth
...
```math
\begin{align*}
t' = \frac{1}{\mu} log\frac{(K-N_0\pm \delta N)N }{(K-N)N_0\pm \delta N}
\end{align*}
```
...
```math
\begin{align*}
t' - t  = \frac{1}{\mu} (log\frac{(K-N_0\pm \delta N)N }{(K-N)N_0\pm \delta N} -  log\frac{(K-N_0)N}{(K-N)N_0})\\
t' - t  = \frac{1}{\mu} (log\frac{\frac{(K-N_0\pm \delta N)N }{(K-N)N_0\pm \delta N}}{\frac{(K-N_0)N}{(K-N)N_0}})\\
t' - t  = \frac{1}{\mu} (log\frac{\frac{(K-N_0\pm \delta N)\cancel{N} }{\cancel{(K-N)}N_0\pm \delta N}}{\frac{(K-N_0)\cancel{N}}{\cancel{(K-N)}N_0}})\\
\end{align*}
```
next
```math
\begin{align*}
t' - t  = \frac{1}{\mu} (log\frac{(K-N_0\pm \delta N) N_0}{(K-N_0) \cdot (N_0\pm \delta N)})\\
t' - t  = \frac{1}{\mu} log\frac{K \cdot N_0 - N_0 \cdot (N_0\pm \delta N) }{K \cdot (N_0\pm \delta N) -N_0 \cdot (N_0\pm \delta N) }\\
\end{align*}
```
dividing numerator and denominator by $N_0 \cdot (N_0\pm \delta N)$
...
```math
\begin{align*}
t' - t  = \frac{1}{\mu} log\frac{\frac{K \cdot N_0}{N_0 \dot (N_0\pm \delta N)} - 1 }{ \frac{K \cdot (N_0\pm \delta N)}{N_0 \cdot (N_0\pm \delta N)} -1 }\\
t' - t  = \frac{1}{\mu} log\frac{\frac{K \cdot \cancel{N_0}}{\cancel{N_0} \dot (N_0\pm \delta N)} - 1 }{ \frac{K \cdot \cancel{(N_0\pm \delta N)}}{N_0 \cdot \cancel{(N_0\pm \delta N)}} -1 }\\
t' - t  = \frac{1}{\mu} log\frac{\frac{K}{N_0\pm \delta N} - 1 }{ \frac{K}{N_0} -1 }\\
t' - t  = \frac{1}{\mu} log\frac{\frac{K \cdot \cancel{N_0}}{\cancel{N_0} \dot (N_0\pm \delta N)} - 1 }{ \frac{K \cdot \cancel{(N_0\pm \delta N)}}{N_0 \cdot \cancel{(N_0\pm \delta N)}} -1 }\\
t' - t  = \frac{1}{\mu} log\frac{\frac{K}{N_0\pm \delta N} - 1 }{ \frac{K - N_0}{N_0}}\\
\end{align*}
```
now 
...
\begin{align*}
t' - t  = \frac{1}{\mu} log\frac{\frac{K}{N_0\pm \delta N} - 1 }{ \frac{K - N_0}{N_0}}\\
t' - t  = \frac{1}{\mu} log\frac{N_0(\frac{K}{N_0\pm \delta N} - 1 )}{K - N_0}\\
t' - t  = \frac{1}{\mu} log\frac{(\frac{K}{\frac{N_0\pm \delta N}{N_0}} - N_0 )}{K - N_0}\\
t' - t  = \frac{1}{\mu} log\frac{(\frac{K}{1 \pm \frac{\delta N}{N_0}} - N_0 )}{K - N_0}\\
\end{align*}
```


##
In conclusion, for the logistic growth, we obtain the same effect of perturbations being more important when reducing than when increasing the population. On the other hand, we have an additional modulation caused by the value of the carrying capacity. 
