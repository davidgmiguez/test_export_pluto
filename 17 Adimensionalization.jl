...
# ╔═╡ 1b4d075d-d9c4-43ad-bb72-9d253ef88473
begin
	struct Foldable{C}
	    title::String
	    content::C
...
md"
# Nondimensionalization of systems of differential equations:

When Michaelis and Menten performed their famous aproximation, they did it because they knew the particular feature of their system, that complex formation and dissociation have a much faster dynamics that formation of product. 
##
...
##
The answer is yes, we can do this based on separations on time scales: if reactions are very fast or very slow compared to another reaction, we can assume that they are in equilibrium. But, how do we know if reactions are faster or slower? if we cant do it looking at the rate constants.
...
- first we have to be sure that we are comparing apples with apples. Not all rate constants are created equal. 
- second, and most important, we know that the speed of reactions depends on the rate  and the concetrations of reactants. 

##
One useful way is to normalize all variables, so they change from a minimum value of 0 to a maximum value of 1. 

here we will learn the basics of normalization and adimensionalization, that will allow us to reduce the system to its natural scales. In other words, we will learn how to do a change of variable that will result in a much simpler and clear model. Since variables in this new model has no dimensions, aproximations are much easier. 

Let's practice with a simple example of a single ODE:

```math
\begin{align*}
  a \dot{x} + b x = c F(x,t) \tag{1}
  \end{align*}
```

the system is composed of three constants $a$, $b$ and $c$,  each one with its own units (so we cannot directly compare them), and two variables $x$ and $t$. 

The firts step is to find new res-scaled variables $\chi$ and $\tau$, defined as
...
\begin{align*}
  ...
  \end{align*}
```
where $x_c$ and $t_c$ are constants. Constant $x_c$ is associated with a natural concentration of the variable $x$, which is typically a maximum or a minimum value. Constant $t_c$ is associated with a natural time scale of the system, such as a characteristic time.   

##

As a consequence of how we defined the new variables, $\chi$ and $\tau$ will have no dimensions. If we write the derivative now:

```math
\begin{align*}
  \frac{\delta x}{\delta t} = \frac{\delta (\chi \cdot x_c)}{\delta (\tau \cdot t_c)} = \frac{x_c}{t_c}\frac{\delta \chi}{\delta \tau} 
  \end{align*}
```

##

now we introduce this in eq (1).
```math
\begin{align*}
\frac{x_c}{t_c} a \frac{\delta \chi}{\delta \tau} + b \cdot \chi \cdot x_c = c F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
...
```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \frac{t_c}{x_c}  \frac{b}{a} \cdot \chi \cdot x_c = \frac{c}{a} \frac{t_c}{x_c}F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
so
...
```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \frac{t_c}{\cancel{x_c}}  \frac{b}{a} \cdot \chi \cdot \cancel{x_c} = \frac{c}{a} \frac{t_c}{x_c}F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
so
```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \frac{b}{a} t_c   \cdot \chi  = \frac{c}{a} \frac{t_c}{x_c}F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
From here, we can try to eliminate the firts constants by writting 
```math
\begin{align*}
\frac{b}{a} t_c = 1 \rightarrow  t_c = \frac{a}{b}
  \end{align*}
```  

meaing that, if we choose $t_c$ in this way, we simplified what is left of the equal sing as 

```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \chi  = \frac{c}{a} \frac{t_c}{x_c}F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
This way, $t_c$, which rememeber has units of time, can be written in terms of teh constans ($\frac{a}{b}$ has also units of time by definition). Now for the right side, we substitute the definition of $t_c$ 
```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \chi  = \frac{c}{a} \frac{a}{b} \frac{1}{x_c}F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
so 
...
```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \chi  = \frac{c}{\cancel{a}} \frac{\cancel{a}}{b} \frac{1}{x_c}F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
From here, we can try to eliminate the firts constants by writting 
...
```math
\begin{align*}
\frac{c}{b} \frac{1}{x_c} = 1 \rightarrow  x_c = \frac{c}{b}
  \end{align*}
``` 

now finally. The equation can be written as 

```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \chi  = F(\chi \cdot x_c,\tau \cdot t_c)
  \end{align*}
```
which is a much simpler formula, we can also substite into the arbitrary function F

```math
\begin{align*}
 \frac{\delta \chi}{\delta \tau} + \chi  = F(\chi \frac{c}{b},\tau \frac{a}{b})
  \end{align*}
```

and depending on the formula, we can maybe define a new function that incorporates these constant values. 

"

# ╔═╡ 6e71935f-f1d0-46d8-9bb7-4c3f4ecdf68e
 md"##
>👉 **Task**: Suppose that a quantity x(t) changes in time according to the ODE
>```math
>\begin{equation}
>\frac{dx}{dt}  = a + bx +cx^2
>\end{equation}
>```
>write a nondimensional form of the equation. Assuming x has units of length (say, meters) and t has units of time (e.g. seconds), what units do the coefficients a, b, and c have?  "

# ╔═╡ 53525fc7-0bac-4618-ab7c-ed792dc74741
Foldable("Click here to reveal the solution:",md"As a starting point, we choose a constant value for $x$, let's say $x_c$, and we let this value be our unit. The ratio
```math
\xi = \frac{x}{x_c}
```
has no units. In the same way we can pick a unit of time $t_c$ and introduce the quantity
```math
\tau = \frac{t}{t_c}
```
which also has no units. The derivative is now 
```math
\frac{dx}{dt} = \frac{x_c}{t_c} \frac{d\xi}{d\tau}
```
and the other side is now:
```math
a+bx+cx^2 = a + (bx_0)\, \xi + (cx_0^2)\, \xi^2
```
so, the full equation is now:

```math
\frac{x_c}{t_c} \frac{d\xi}{d\tau} 
=
a + (b  x_c)\, \xi + (c  x_c^2)\, \xi^2,
```
organizing all variables into one side
```math
\frac{d\xi}{d\tau} = 
\frac{t_c a}{x_c} + (t_c  b) \, \xi + (t_ c  x_c  c)\, \xi^2.
```
At this point we choose $x_c$ and $t_c$. We can try to make the constant term and the coefficient of $\xi$ both equal to 1. If a≠0 and b≠0 then this is possible provided we choose
```math
t_c = \frac{1}{b}, \qquad 
x_c = at_c = \frac{a}{b}.
```
the coeficient of $\xi^2$ becomes

```math
\alpha = t_c x_c^2  c = \frac{a^2 c}{b^3},
```
and we get the following differential equation for $\xi$ as a function of $\tau$:

```math
\begin{equation}
\frac{d\xi}{d\tau} = 1+\xi+\alpha\, \xi^2.
\end{equation}
```

This is a nondimensionalized version of the initial equation. Note that instead of three undetermined parameters (a, b, c) it only has one parameter, namely $\alpha$.

"
)

# ╔═╡ 6fb05f20-f339-4f5c-8b18-502f06d91674
md"
## Normalization of the Enzyme-Substrate system 

Let's learn how to normalize now in the context of Enzyme-Substrate reactions, since this can help us to visualize which variables are faster or slower
##
...
# ╔═╡ baad5358-58ad-498d-b72e-1f87c3ae9ad8
# ╠═╡ show_logs = false
enzyme_kinetics3! = @ode_def abcdefgh begin
...
so, now rewriting the equation as a rescaled variable, we obtain: 
...
```
##
An important but subtle point to note is that the rate constants also change, allowing us to compare directly the rates of substrate and complex formation"
...
manifest_format = "2.0"
project_hash = "34f9be34533ea616af0d2ec71c01c2ca7023e57a"
