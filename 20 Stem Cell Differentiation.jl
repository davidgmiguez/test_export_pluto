...
pp_dd_
	P0_slide = @bind P₀ html"<input type=range min=1 max=200 step=10>"
	D0_slide = @bind D₀ html"<input type=range min=1 max=100 step=10>"
	pp_dd_slide = @bind pp_dd html"<input type=range min=-0.9999 max=1 step=0.05>"
...
begin
		t=collect(0:10)
		plot(t,t->P₀*(1+pp_dd)^t,label="P",seriestype=:line)
		plot!(t,t->D₀+P₀*(((1+pp_dd)^t)-1)*((1-pp_dd)/(pp_dd)),label="D",seriestype=:line,ylims = (0,1000))
		plot!(t,t->P₀*(1+pp_dd)^t+D₀+P₀*(((1+pp_dd)^t)-1)*((1-pp_dd)/(pp_dd)),label="T",seriestype=:line,ylims = (0,1000), title ="Discrete branching process", xlabel ="Number of Generations",ylabel ="Number of Cells")
end
...
Finally, if we rewrite  $\Delta$$P=P_{t}-P_{0}$, we obtain the expression. 
...
begin
	...
	pp_dd_slide_ = @bind pp_dd_ html"<input type=range min=-0.9999 max=1 step=0.05>"
	md"""
	**Set the value:**
	
	value of T: $(T_slide)
	value of pp-dd: $(pp_dd_slide_)
...
begin
		plot(t,t->P₀*(1+pp_dd_)^(t./T),label="P",seriestype=:line)
		plot!(t,t->D₀+P₀*(((1+pp_dd_)^(t./T))-1)*((1-pp_dd_)/(pp_dd_)),label="D",seriestype=:line,ylims = (0,1000))
		plot!(t,t->P₀*(1+pp_dd_)^(t./T)+D₀+P₀*(((1+pp_dd_)^(t./T))-1)*((1-pp_dd_)/(pp_dd_)),label="T",seriestype=:line,ylims = (0,1000),title ="Continuous Branching Process", xlabel ="time [hours]",ylabel ="Number of Cells")
end

# ╔═╡ 31852d81-b553-4b70-8ebd-dab91ba86756
md" ### Including growth fraction

Next, we will take into account the fact that some progenitors do not cycle. Thsi is what it is often refered as quiescence, a concept in biology that refers to the fact that cells are reversibly arrested at some point in the cell cycle, but remain capable of re-entering cell cycle progression upon receiving an appropriate stimulus. In the context of Developmental Systems, the fraction of progenitors cells that are actively cycling is refered as the _growth fraction_, denoted as $\gamma$. 

To include the fact that only $\gamma \cdot P_0$ progenitors are cycling, we modify the equations as:


```math
\begin{eqnarray}
P_1&=&P_{0} \gamma (2pp+pd) \tag{35}\\
D_1&=&D_{0}+P_{0}\gamma(2dd+pd)\tag{36}
\end{eqnarray}
```
carrying the same development of the branching, the final equations where the fact that only some progenitiors are actively cycling are:

```math
\begin{eqnarray}
P_{t}&=&P_0 (1+\gamma (pp-dd))^{\frac{t}{T}}\tag{59}\\
\end{eqnarray}
```

while the equation for differentiated remains equivalent.

"

# ╔═╡ 90640de9-9a3c-4de9-a59e-81d8c653a56e
begin
	γ_slide = @bind γ html"<input type=range min=0.0 max=1.0 step=0.1>"
	
	md"""
	**Set the value of the growth fraction**
...
# ╔═╡ f2a3bc25-4d1a-4a5d-a653-9c443c3731a3
begin
		plot(t,t->P₀*(1+γ*(pp_dd_))^(t./T),label="P",seriestype=:line)
		plot!(t,t->D₀+P₀*(((1+γ*(pp_dd_))^(t./T))-1)*((1-pp_dd_)/(pp_dd_)),label="D",seriestype=:line,ylims = (0,1000))
		plot!(t,t->P₀*(1+γ*(pp_dd_))^(t./T)+D₀+P₀*(((1+γ*(pp_dd_))^(t./T))-1)*((1-pp_dd_)/(pp_dd_)),label="T",seriestype=:line,ylims = (0,1000),title ="Continuous Branching for growth fraction = $γ", xlabel ="time [hours]",ylabel ="Number of Cells")
end

# ╔═╡ 3e8066bd-d8dc-47d9-98e5-0b8b0759e179
md" ### Including Apoptosis

Next, we will take into account the fact that some progenitors can die. This is defined as the _rate of apoptosis_, denoted as $\phi$ as the number of cells that die per cell cycle. 

Now the equation for progenitors is results in:

```math
\begin{eqnarray}
P_{t}&=&P_0 (1 - \phi +\gamma (pp-dd))^{\frac{t}{T}}\tag{59}\\
\end{eqnarray}
```
...
# ╔═╡ 48f81c5d-7dee-4c7c-8251-3f2191098d55
begin
	ø_slide = @bind ø html"<input type=range min=0.0 max=1.0 step=0.1>"
	
	md"""
	**Set the rate of apoptosis**
...
# ╔═╡ c6233b75-32c2-4d78-a924-f62082c7edef
begin
		plot(t,t->P₀*(1-ø+γ*(pp_dd_))^(t./T),label="P",seriestype=:line)
		plot!(t,t->D₀+P₀*(((1-ø+γ*(pp_dd_))^(t./T))-1)*((γ-γ*pp_dd_)/(γ*pp_dd_-ø)),label="D",seriestype=:line,ylims = (0,1000))
		plot!(t,t->P₀*(1-ø+γ*(pp_dd_))^(t./T)+D₀+P₀*(((1-ø+γ*(pp_dd_))^(t./T))-1)*((γ-γ*pp_dd_)/(γ*pp_dd_-ø)),label="T",seriestype=:line,ylims = (0,1000),title ="Continuous Branching for apoptosis = $ø", xlabel ="time [hours]",ylabel ="Number of Cells")
end

# ╔═╡ e253ec6c-8c1b-4712-8a86-9e499e347282
md"### Calculating cell cycle length and mode of division

Quite often in a developing population, the values that can be quantified are the numbers of cells of each population, and the question that we want to address is what was the average cell cycle length, and the average mode of division. This values can be easily obtained using the equations above, since they are uncloupled. 


```math
\begin{eqnarray}
    pp-dd = \frac{\phi\Delta D+\gamma\Delta P}{\gamma(\Delta D+\Delta P)}, 
\end{eqnarray} 
```

with $\Delta D = D_n - D_0$ and $\Delta P = P_n - P_0$. 

For the average length of the cell cycle of the cycling progenitors we obtain:
```math
\begin{eqnarray}
    T = t \cdot \frac{\log(1-\phi+\gamma(pp-dd))}{\log\frac{P(t)}{P_0}}
\end{eqnarray}
```

We test these equations in the next plot, to see that they work as expected. 


"

# ╔═╡ e19cd02f-6713-4024-ac0e-61d9449c529f
function row_diff(a)
    b = similar(a)
    for i in 2:size(a,1)
         b[i] = a[i] ./ a[i-1]
...
# ╔═╡ 49dab4da-650d-497d-a118-a45006046827
function estimated_dynamics(P,D,γ,ø,t)
    pp_dd = similar(P)
	T = similar(P)
	
        pp_dd = (γ.* diff(P) + ø .* diff(D)) ./ (γ.* (diff(P) .+ diff(D)));
 		T = diff(t) .* log.(1 .-ø .+(γ.*((pp_dd)))) ./ (log.(row_diff(P)[2:end]));
    return [pp_dd,T]
end

# ╔═╡ 88a16c2f-4c6f-43d2-b167-11eed3ba04f5
begin
	
	P1=P₀.*(1 .- ø .+ γ .*(pp_dd)).^(t./T)
	D1=D₀ .+ P₀ .* (((1 .- ø .+ γ .* (pp_dd)).^(t./T)) .- 1) .* ((γ .-γ .* pp_dd) ./ (γ .*pp_dd .-ø))
	...
	#pp_dd1=(γ.* diff(P1) + ø .* diff(D1)) ./ (γ.* (diff(P1) .+ diff(D1)));
	#T1_= diff(t) .* log.(1 .-ø .+(γ.*((pp_dd1)))) ./ (log.(row_diff(P1)[2:end]));


	#pp_dd1=(γ.* (P1_ .-P1__) + ø .* (D1_ .-D1__)) ./ (γ.* (P1_ .-P1__ .+D1_ .-D1__));
	#T1_=(t_ .-t__) .* log.(1 .-ø .+(γ.*((pp_dd1)))) ./ (log.(P1_ ./P1__));

	P4=scatter(t[2:end],estimated_dynamics(P1,D1,γ,ø,t)[1], ylims = (-1,1),label=" Estimated pp-dd")
	hline!([pp_dd],label=" true pp-dd",title ="Mode of division", xlabel ="time [hours]",ylabel ="pp-dd")

	P5=scatter(t[2:end],estimated_dynamics(P1,D1,γ,ø,t)[2],ylims = (0,T*2),label=" Estimated T")
	hline!([T], label=" True T",title ="rate of division", xlabel ="time [hours]",ylabel ="T [hours]")

	plot(P4,P5,layout=(1,2),legend=true,size = (800, 500))
	
end

# ╔═╡ 86fb2696-e17a-481e-99fa-41052688c63e
md"### Simulations "

# ╔═╡ 574eae35-2677-4d2a-acc1-874b23501e8c
begin

	number_of_generations=11;number_of_simulations=10; 
	GsP = zeros(number_of_simulations,number_of_generations);#allocate Progenitors
	GsD = zeros(number_of_simulations,number_of_generations);#allocate Differentiated 
	GsP[:,1] .= P₀;
	GsD[:,1] .= D₀;
	
	for j = 1:number_of_simulations
	    for ii = 2:number_of_generations
			# we assing a random number to all progenitors in every iteration
	        r = rand(Int(round(GsP[j,ii-1])))
			# we calculate how many progenitors die in this iteration
			cells_dying = ø * size(r)[1];
			# we find how many progenitors divide via pp
			cells_dividing_pp =γ * size(findall(r .<= (1+ pp_dd)/2))[1];
			# we find how many progenitors remain quiescent
			cells_not_dividing = (1-γ) * size(r)[1];
			# we see how many progenitors divide via dd
			cells_dividing_dd = γ * size(findall(r .>= (1+ pp_dd)/2))[1];
			
	        GsP[j,ii] = 2 * cells_dividing_pp + cells_not_dividing - cells_dying
			GsD[j,ii] =  2 * cells_dividing_dd + GsD[j,ii-1]
	    end
	end
	
	pl1 = plot()
		for ii = 1:size(GsP,1)
		     pl=plot!(collect(0:number_of_generations-1), GsP[ii,:], c = :green, line = :dash, label = false, xticks = 0:1:15, xlims = (0,number_of_generations))
			pl=plot!(collect(0:number_of_generations-1), GsD[ii,:], c = :red, line = :dash, label = false, xticks = 0:1:15, xlims = (0,number_of_generations))
			
		end
	
		pl1 = plot!(title = "Population Dynamics",ylims = (0,1000))
		n = collect(0:number_of_generations)
	        pl1 =plot!(n,n->P₀*(1-ø+γ*(pp_dd))^n,label="P", c = :green,seriestype=:line)
			pl1 =plot!(n,n->D₀+P₀*(((1-ø+γ*(pp_dd))^n)-1)*((γ-γ*pp_dd)/(γ*pp_dd-ø)),label="D",seriestype=:line, c = :red)
			#plot!(n,n->P₀*(1+pp-dd)^n+D₀+P₀*(((1+pp-dd)^n)-1)*((1-pp+dd)/(pp-dd)),label="T",seriestype=:line)
	
end

# ╔═╡ 73e8f62c-48ce-45f6-bb7b-f09cca472f6e
begin
	P6=scatter(t[2:end],estimated_dynamics(GsP[4,:],GsD[4,:],γ,ø,t)[1], ylims = (-1,1),label=" Estimated pp-dd")
		hline!([pp_dd],label=" true pp-dd",title ="Mode of division", xlabel ="time [hours]",ylabel ="pp-dd")
	
		P7=scatter(t[2:end],estimated_dynamics(GsP[1,:],GsD[1,:],γ,ø,t)[2],ylims = (0,T*2),label=" Estimated T")
		hline!([T], label=" True T",title ="rate of division", xlabel ="time [hours]",ylabel ="T [hours]")
	
		plot(P6,P7,layout=(1,2),legend=true,size = (800, 500))
end

# ╔═╡ f33010ce-5397-43f0-9ea5-903f2fbb9beb
estimated_dynamics(GsP[4:5,:],GsD[4:5,:],γ,ø,t)[1]

# ╔═╡ 403d551f-4d1f-4831-ade1-47ab7257b2d8
diff.(GsP[4:5,:])

# ╔═╡ 9e59a30a-3b22-4ced-a561-6c192ea4acca
md"now we can perform simulations with the cell cycle also, in continuous"
...
begin
	...
	P=P₀.*(1+pp_dd).^(t./T)
	D=D₀.+P₀.*(((1 .+pp_dd).^(t./T)).-1).*((1-pp_dd)/(pp_dd))
	...
	T_=(t_ .-t__) .* log.(1 .+(1 .*((pp_dd)))) ./ (log.(P_ ./P__));

	P2=scatter(t_,pp_dd, ylims = (-1,1),label=" Estimated pp-dd")
	hline!([pp_dd],label=" true pp-dd",title ="Mode of division", xlabel ="time [hours]",ylabel ="pp-dd")
	
	P3=scatter(t_,T_,ylims = (0,T*2),label=" Estimated T")
	hline!([T], label=" True T",title ="Rate of division", xlabel ="time [hours]",ylabel ="T [hours]")
...
manifest_format = "2.0"
project_hash = "902724cccaec4e0dd159d171d51db1d31bfbbda1"
