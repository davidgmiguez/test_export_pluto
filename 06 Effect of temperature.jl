...
begin
	...
	md"""
	**Move the slider to change	the activation energy Ea**: $(E_slide)
...
md"##
In conclusion, increasing the energy of activation reduces the rate constant of the reaction but increases its dependence on the temperature. This makes sense because if more energy is required to cross the barrier, the dependence on the property that allows you to cross that barrier increases. "
...
# ╔═╡ 82401181-596e-4903-a1ef-489ac86c722c
begin
	E_slide2= @bind E_a2 html"<input type=range min=0.4 max=2.0 step=0.1>"
	md"""
Below, we plot the typical form of the ∆G for both types of reaction, you can move the silder to change the Ea: $(E_slide2)		
		"""
end
...
##
To see this that, let's apply logs to obtain the Arrienous equation in the form of a straight line
...
```
##
Now, let's plot the two kinetic rate constants for a reversible reaction. Assuming that the forward reaction is the exothermic one, and therefore, the reverse should be endothermic by definition. This is established here by setting the $E_a$ for the forward half of the $E_a$ of the reverse reaction. 
"
...
begin
	...
	E_slide_ = @bind E_a__ html"<input type=range min=1.0 max=20 step=1>"
...
begin
	p5=plot(1 ./ T,log(A_f).-(E_a__./2 ./ (0.082.*T)),ylims = (-2,0),label = "k1, Exothermic")
	p5=plot!(1 ./ T,log(A_r).-(E_a__./(0.082.*T)),ylims = (-2,0),label = "k2, Endothermic")
	...
	p9=plot(T,A_f.*exp.(-E_a__./2 ./(0.082.*T))./(A_r.*exp.(-E_a__ ./(0.082.*T)) ),label = "Equilibrium Constant",ylims = (0,5))
...
md" ##
In the left panel, we can see that both $k_1$ and $k_2$ increase with the temperature, but the dependence is always stronger for the endothermic. Therefore, increasing the temperature will result in a shift of the equilibrium towards the endothermic reaction (the reaction that consumes heat). 

##
In other words, if more heat is available, then the reaction that requires heat is favored, and the equilibrium is shifted based on this change. On the contrary, a decrease in temperature favors the reaction that increases the temperature.

In the right panel, we can see that, because a change in the temperature affects different forward and reverse reactions of a reversible reaction, a change in the temperature will shift the equilibrium constant, as expected. 
...
md"## 
Another example can be developed taking into account that we are learling an interdisciplinary subject, so let's try to develop a model that focuses on the formation of collaborations between physicists and biologists. 
...
# ╔═╡ dc642c8f-ef39-4bec-9c57-c9465c4a1b48
md"lets plot now the two kinetic constants, and the equilibrium constant."
...
manifest_format = "2.0"
project_hash = "a3b9afd8b9dffd57e1aca1023bf4ba92d62973de"
