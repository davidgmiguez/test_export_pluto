...
# ╔═╡ 6a96191c-81b9-4ce6-b96c-b28331998047
...
md" 
##
A key aspect of complex systems is their dynamics, i.e., how they change as time progresses. In this chapter, we will start to define simple mathematical models that can help us to study and predic the  evolution of biological systems in time. This way, these mathematical models can be identified as abstract descriptions of biological systems using mathematical concepts.  

##
So, when we try to capture the change in the state of a given system overtime, we have to include the temporal variable. Therefore, our basic tool from now own will be some sort of differential equation, or sets of differential equations that can compute how the system changes between two different time points. 

##
!!! note
	What type of models do we want? Very realistic models with many interactions or simpified models?

	Realistic models with many variables have the potential to reproduce more faithfully the real system, but simple models have a key advantage, they can be much easily analyzed and understood. Realistic models become obscure very fast and do not provide much information of the experiment. Contrary to one may think, simple models with few equations are much more difficult to generate than realistic models with many interactions. The ideal model is one that can reproduce the core properties and features of a given experiment, with a minima set of variables.
...
md"
# 3.1 Unconstrained growth in a bacterial population 

The most basic approach to bacterial population growth is to begin with the assumption that every individual duplicates via mitosis, which would double the population size each generation. 
##
...
# ╔═╡ 3b4049b9-fc37-422a-a146-84ccc4903bee
begin
	r_slide = @bind r html"<input type=range min=0.8 max=1.3 step=0.05>"
...
# ╔═╡ 3ca67c6e-8bee-42c1-8df1-08c6b7ca1b45
time=collect(1:100);

# ╔═╡ 07adcb41-88f3-4433-b238-a50fbe35985c
...
Next, we introduce the concept of carrying capacity _K_, which represents the maximum population size that a particular environment can support. This means that when the system reaches its carrying capacity, it stops growing, and reaches an equilibrium. In other words, r(K) = 1
##
...
```
Therefore, the function $r(p)$ takes the form
...
# ╔═╡ e762e3b6-e5ae-46d9-8313-3648e226dcef
function unconstrained_growth_generic(p, r; dt=0.01)
traj = []
	slope= (1-r)
for t in 1:100 # arbitrary, just leave enough for it to reach a steady state given the dt
	#p += dt * (p * ((1.10 - 0.002 * p) - 1))	
	p = p * (r)
#p = p * r * (1 - p/K + p/(r*K)) # otra forma
...
# ╔═╡ c1fee67b-9218-400c-99c6-7e7f4f5e4e12
...
# ╔═╡ 115f3482-cebc-41ae-969a-318efdc0bd87
begin
		rrrr_slide = @bind rrrr html"<input type=range min=0.1 max=1.7 step=0.1>"
		
		md"""
		## 
		In the next plot, you can visualize how the number of cells changes over time for different values of the $r_0$ by moving the slider: $(rrrr_slide)
		
		"""
end

# ╔═╡ ce0b4683-513f-4b31-bee0-f59686aae9f6
begin
	plot(time,constrained_growth_generic(5, rrrr,50; dt=0.01),ylims = (0,55),xlabel=("Time"),ylabel=("Number of cells"),title=("Constrained growth, r=$rrrr"),label=("Constrained growth"))
	
	plot!(time,unconstrained_growth_generic(5, rrrr; dt=0.01),ylims = (0,55),xlabel=("Time"),ylabel=("Number of cells"),title=("Unconstrained vs constrained, r=$rrrr"),label=("Unconstrained growth"))
end

# ╔═╡ cf975d0e-48ed-4bf2-a629-f03eb26d3180
md"You can see that there is critical point in $r_0 = 1$, where a small variation changes drastically the system, from extintion of the colony to exponential expansion. "
...
# ╔═╡ 51b803bc-aff5-4296-95e1-8323c3618aa7
...
# ╔═╡ a0f86dc2-93a0-4ff5-b2f5-c7e80550f1e3
...
# ╔═╡ d8e7ce8a-1ff8-48c9-9a0d-2e8f359bd153

Foldable("Plot for Task 1a:", plot([1:10],constrained_growth_1a(100, 2 ,-0.002,10),ylims = (0,500),xlabel=("Time"),ylabel=("Number of cells"),title=("Constrained growth, r=$rrrr")))


# ╔═╡ a88891a1-23d2-4352-83ca-45eca48ca885
...
# ╔═╡ 6697145c-73f4-43be-a458-d1ac9026ed19
Foldable("Plot for Task 1b:",plot([1:10],constrained_growth_1a(300, 2 ,-0.002,10),ylims = (0,500),xlabel=("Time"),ylabel=("Number of cells"),title=("Constrained growth, r=$rrrr")))
...
# ╔═╡ 63e41329-da81-4e6a-af87-ef18949f696d
Foldable("Solution 2a:", md"the intial r=2, and it goes to 1 when p=10^6
...
# ╔═╡ 1dd19aa2-c4ad-47fd-8f11-6aa1b2679f40
...
# ╔═╡ 078f3e98-b370-4b07-ba42-2e25e0963c55
begin
...
# ╔═╡ 6f3c4c98-da0b-4cd4-8052-a74abe75d22a
plot([1:50],constrained_growth_4(1, r; dt=0.01),ylims = (0,50000+1E6),xlabel=("Time"),ylabel=("Number of cells"),label=("100% proliferating"),title=("Constrained growth, r=2"))

# ╔═╡ 6979bee0-db80-4dc1-a9f4-19e3f1470c5f
Foldable("Solution 2b:", md"(b) Solution: so, now on average, from 100 cells, only 50 cells proliferate. This is a common biological phenomena called _quiescence_, by which some cells in a population decide not to cycle and rest. Sometimes thsi rest can last years (adult stem cells). 
...
# ╔═╡ 116511f2-7585-4e73-8b94-11025cc365ec
begin
...
# ╔═╡ 72ff745b-c36a-4999-9e1e-7a9c7ce15664
...
# ╔═╡ 57d9e299-7deb-48eb-9b18-11d09a4e9f6b
begin
...
# ╔═╡ f581b4d8-de0e-450d-b2c2-1431bc99d606
md"so, you can see that you reach the same final point than in 2a, but later in time"

# ╔═╡ ae23309d-cfa7-48f7-965a-5bb3a054a0f0
Foldable("Solution 2c:", md"If counting all cells, 10% died, it means that, if we start with 100, we generate 150. Then 10% of the cells die, so in the first iteration with no restrictions you produce 135, this means an r=1.35.")

# ╔═╡ 1ede2ba1-18d6-4385-9df1-458b572e5121
Foldable("Solution 2d:", md"if from the newborn cells, only 90% survive, this means that if we start with 100 cells, and 50% proliferate, we obtain 50 new cells. Now, from these new 50 cells, only 45 cell survive. from 50 cells that are non cycling, we have to remove the 20%, so we have 40 cells. In total, from 100 cells, you have 50 mothers, 45 daugthers, and 40 noncycling, so 135 in total. this gives an r=1.35")
...
```math
x_{t+1} - x_{t} = 0 = x_{t} \cdot r (1 - x_{t}) - x_{t} 
```
...
```math
x_{t}  = r \cdot x_{t}  (1 − x_{t}) 
```
##

Simplifying the notation  $x_{t}$ = $x$, and solving for $x$, we find that the
...
# ╔═╡ 00d83443-83c5-4852-a367-163613e6f66b
begin
...
# ╔═╡ d91a8f2e-619c-403e-887f-e233e440f487
plot(time,logi, xlims=(0,100),ylims=(0.0,1.0),xlabel="Time",ylabel="Number of individuals in population",title=("Constrained growth, r=$rrrrr"),leg=false)
...
md"now we will plot on top of this, the first time points of the simulation of the population growth. This type of plot is called a _Verhulst diagram_."
...
manifest_format = "2.0"
project_hash = "baddd76310b8118e5911e42131aeafe0cfeb3dc7"
