...
using HypertextLiteral, PlutoUI, DifferentialEquations, ParameterizedFunctions, Plots, Plots.PlotMeasures

# ╔═╡ c6a39876-2a02-4b0a-8483-cd91fe629510
begin
	struct Foldable{C}
	    title::String
	    content::C
...
# ╔═╡ ae4241e5-c36f-427b-9a46-f61f4eaf0693


# ╔═╡ fee3f3ef-a9a2-4bb7-965d-46c0ad7d289c
Resource("https://schoolbag.info/chemistry/mcat_2/mcat_2.files/image357.jpg",:width => 300)
...
begin
	...
	md"""
	##
	**Move the silder to change the value of
	Ea**: $(E2_slide)
...
begin
	...
	plot(P1,P2,layout=(1,2),legend=true,size = (800, 300),left_margin = [10px 0px], bottom_margin = 15px)
end
...
Resource("https://1.bp.blogspot.com/-ttv1gwKAvsc/X8Y4zo6S6lI/AAAAAAAAAQg/LkU50ozS8lsKPsnZ6keV_FU1binSjIAxwCLcBGAsYHQ/w1200-h630-p-k-no-nu/what%2Bis%2Benzyme%2Bin%2Bbiology.PNG",:height => 200)
...
begin
	...
	md"""
	##
	**Move the silder to change the value of Ea:** $(E_slide)
...
md"##
This is one of the reasons underlying the observed robustness of biological systems to changes in the temperature, since most of the reactions that take place in biological processes are calalyzed by enzymes."
...
These type of reactions typically occurs in two steps. The first step corresponds to the formation of an intermediate complex $c$ of the enzyme $e$ and the substrate $s$. This firts step is considered as reversible, so the complex $c$ can dissasemble or react in a second step to form the product $p$. In this second step, we also recover the enzyme $e$, since it is not consumed in the reaction process and only acts as a catalist. 
...
md" 
##
> __Task 1__: Generate the Differential equations for the Enzyme-substrate system, and solve them numerically for the following parameter values: $k_1$=1e3, $k_2$=0.1, $k_3$=0.05. Use the following initial conditions: $e_0$=$s_0$=0.002. For the simulation time use 100 seconds"

# ╔═╡ 5748487d-8779-41c6-83ab-94fbea05351a
Foldable("Click to reveal solution of Task 1:",md"
##
...
```
##
To solve this system numerically, we define a function with this differential equations that then solve numerically using iterative calculations or a built in ODE solver. In Julia, the function with the equations can be written using the simpler DSL notation. A main advantage of this approach is that it uses directly the differential equations with the original names of the variable, reducing the need of vector indexes. To do this, we substitute now the names of the variables (in Julia, we cannot use $e$ as the name of a variable, so we use $en$):
...
# ╔═╡ 152e0ff0-db07-4750-9303-411d6e4f177a
begin
	k1=1e3
	k2=0.1
	k3=0.05
	e0=0.002
	s0=0.002
	prob0 = ODEProblem(simpleODEEnzyme!,[s0,0,e0,0],(0.0,100),[k1,k2,k3])
	sol0 = solve(prob0)
	p0=plot(sol0,size = (650, 300),left_margin = [10px 0px], bottom_margin = 15px)
...
# ╔═╡ dfcf23c6-215a-40b7-93e9-18af74772e3e
Foldable("Click to reveal the plot corresponding to Task 1:",p0)
...
begin
	...
	sol1 = solve(prob1)
	p0000=plot(sol1,size = (800, 300),left_margin = [10px 0px], bottom_margin = 15px)
...
# ╔═╡ 978301a3-2b30-4a80-b2ba-3a3e8ccfc9c0
Foldable("Click to reveal the plot corresponding to Task 1 (DSL method):",p0000)

# ╔═╡ 435a8401-1c27-4a85-bb78-ae8a9c39eeb4
md"
## 8.4 Mass conservation applied to the Enzyme-Substrate system:
...
```
##
which is valid for any value of $C_1$ and $C_3$, so, let's make $C_3 =0$  to obtain the conditions specific for the variables that depend on $C_1$:
...
which means that the sum of this three variabels is a constant, independetly of teh value of $C_1$. This way, if we set this to 1:

```math
\begin{align*}
X_1(t) + X_2(t) + X_4(t) & = cte\\
\end{align*}
```
or, substituting the variables by the real name:
...
which makes sense, since the $s$, $c$ and $p$ are three forms of the same protein, and their sum has to be constant, and equal to the initial value of substrate used. This way: 

```math
\begin{align*}
s(t) + c(t) + p(t) & = cte =s(0) + c(0) + p(0)  \\
\end{align*}
```

And, since we typically start a biochemical reaction by adding only $e$ and $s$:
...
This last equation is our first conservation law. We could use this to eliminate one variable in our system. For instance, could not compute the value of p, and simply calculate it afterwards using the relation:
...
this way, we could rewrite the set of differentiatl equations as 
```math
\begin{align*}
\dot{s}  &= k_2 \cdot c - k_1 \cdot e \cdot s \tag{5}\\
\dot{c} &= k_1 \cdot e\cdot s - (k_2+k_3) \cdot c \tag{6}\\
...
# ╔═╡ 6a9153c4-6f52-4a4d-8fdd-66a702ffcbb7
md" 
##
> __Task 2__: Solve the previous systems of differential equations for the enzyme-substrate system simplified using mass conservation using the same parameters as in the previous task. Calculate the dynamics of the product using the relation obtained by the mass conservation and validate that the result is the same as in the full system with four equations. "

# ╔═╡ d875a18f-8749-4176-a00e-6e3591f4f2b3
	begin
	p00000=plot(sol1,size = (800, 300),left_margin = [10px 0px], bottom_margin = 15px)

	p_solution=[(s0 - u[1] - u[2]) for (u,t) in tuples(sol1)]
	p00000=plot!(sol1.t,p_solution,
	        label="p calculated",        
	        linealpha = 0.5,
	        linewidth = 3,
	        linestyle= :dash,
...
# ╔═╡ 35bdc073-472e-4364-8a15-ff2658b5e66f
Foldable("Click to reveal the plot corresponding to Task 2:",p00000)

# ╔═╡ 53aa50f2-602c-4d40-8d54-377f700cd777
md"
## Second Mass conservation:

For the second, we use the parameters that depend on the other constant $C_3$, so we set the value of $C_1=0$: 
...
which also makes sense, because the amoun of enzyme is not consumed and is only in the form of the reactans $e$ and $c$. 
##
So, giving the value $C_3=1$, we obtain the second conservation law, 
...
which is true because the enzyme is a catalyst that facilitates the reaction but does not react itself, and if we assume that there is no complex $c(0)=0$ before the reaction starts, we can assume ($e(0)+c(0)=e_0$). 
##
Interestingly, we can use this second conservation law to reduce the system of differential equations into the following two coupled ODEs:
...
md" 
##
> __Task 3__: Solve numerically the simplified system using Mass Conservation using the same parameter values. Compare it with the solution of the full system obtained in the first task.
>    
"
...
begin
	prob2 = ODEProblem(enzyme_kinetics2!,[s0,0],(0.0,100.0),[k1,k2,k3,e0,0])
	sol2 = solve(prob2)
	plot(sol2)
	...
	p00=plot!(sol2.t,e,
	        ...
	        linecolor = :purple,
		linestyle= :dash,
		size = (650, 300),left_margin = [10px 0px], bottom_margin = 15px)

	p_solution2=[(s0 - u[1] - u[2]) for (u,t) in tuples(sol2)]
	p222=plot!(sol2.t,p_solution2,
	        label="p calculated",        
	        linealpha = 0.5,
	        linewidth = 3,
	        linestyle= :dash,
	...
	title!("Enzyme-substrate kinetics using mass action")
	xlabel!("Time [s]")
	ylabel!("Concentration [M]");
nothing
end

# ╔═╡ 77dcac4b-9dd9-43c5-b52a-4ab2e455c067
Foldable("Click to reveal the plot corresponding to Task 3:",p00)

# ╔═╡ a04787f9-8262-43dc-921f-be23863a019c
md"## 8.5 Quasi steady state approximation:

The other conservation law could be used to reduce the system to just two equations, but instead, we will use another approximation that allows us to go further and reduce the system to a single differential equation. This aproximation si swidely used in all biochemistry books and has been famously proposed by Michaelis and Menten. 
##
...
begin
	plot(p00)
	c_eq1=[(k1 * e0 * u[1])/(k3+k2+k1 * u[1]) for (u,t) in tuples(sol2)]
	plot!(sol2.t,c_eq1,
	        label="c_eq",        
	        linealpha = 0.5,
	        linewidth = 3,
...
md"
##
As we can see, the $c_{eq}$ value does not correlate with the real value of $c$ at initial time points, but it is quite close to the concentration of $c$ aftre the initial transient. In this conditions we can simplify all equations as:
...
```
##
So the final equation takes the form:
...
```math
  \dot{s}  = - \frac{ k_3 \cdot e_0\cdot s}{\frac{k_3+k_2}{k_1}+s} \tag{30}
```
##
If we solve numerically this enzyme kinetics model reduced to a single equation  using Mass Conservation and the Pseudo-steady state we obtain.


"

# ╔═╡ 3bfc322d-c684-4dd9-ad4e-76b37a7374cb
# ╠═╡ show_logs = false
enzyme_kinetics3a! = @ode_def ab4 begin
	  ds = - (k3 * e0 * s/((k3+k2)/k1 + s))
	    end k1 k2 k3 e0;

# ╔═╡ 3832ca59-af21-42d3-b7c4-20c4dc20df0f
begin
	prob3a = ODEProblem(enzyme_kinetics3a!,[s0],(0.0,100.0),[k1,k2,k3,e0])
		sol3a = solve(prob3a)
		p3a=plot(sol3a,label="s pseudo steady state")

	s_full=[u[1] for (u,t) in tuples(sol2)]
	plot!(sol2.t,s_full,
	        label="s full system",        
	        linealpha = 0.5,
	        linewidth = 3,
		...
		title!("Enzyme-substrate kinetics (Pseudo Steady State)")
		xlabel!("Time [s]")
		ylabel!("Concentration [M]")

end

# ╔═╡ 9da9e889-927c-46f9-a1e5-5bcef21f6dc1
md" It its clear that for this concentrations, the aproximations is not good enought. This is due to the fact that the time-scales required to consider the pseudo-steady state aproximation are not sufficiently different. We can play with the sliders below to find parameter values that resut in a better aproximation. "

# ╔═╡ d814dace-691b-44c7-ad8f-50f5d55d1540
begin
	k1_slide = @bind k1_ html"<input type=range min=1e3 max=1e4 step=10>"
	k2_slide = @bind k2_ html"<input type=range min=0.1 max=1.0 step=0.1>"
	k3_slide = @bind k3_ html"<input type=range min=0.1 max=1.0 step=0.1>"
	md"""
...
# ╔═╡ 90aea824-1abc-4853-b8ff-130cf000be80
begin
		prob3b = ODEProblem(enzyme_kinetics3a!,[s0],(0.0,100.0),[k1_,k2_,k3_,e0])
			sol3b = solve(prob3b)
			p3b=plot(sol3b,label="s pseudo steady state")

		c_full2b=[(k1 * e0 * u[1])/(k3+k2+k1 * u[1]) for (u,t) in tuples(sol3b)]
	     plot!(sol3b.t,c_full2b,
	         label="c pseudo steady state",        
	        linealpha = 0.5,
	        linewidth = 1,
	        linestyle= :solid,
	        linecolor = :orange)
		...
		prob2b = ODEProblem(enzyme_kinetics2!,[s0,0],(0.0,100.0),[k1_,k2_,k3_,e0,0])
		sol2b = solve(prob2b)
	
		s_full2b=[u[1] for (u,t) in tuples(sol2b)]
		plot!(sol2b.t,s_full2b,
		        label="s full system",        
		        linealpha = 0.5,
		        linewidth = 3,
		        linestyle= :dash,
		...
		c_full2b=[u[2] for (u,t) in tuples(sol2b)]
		plot!(sol2b.t,c_full2b,
		        label="c full system",        
		        linealpha = 0.5,
		        linewidth = 3,
		        linestyle= :dash,
...
# ╔═╡ 269eb54b-db05-43d3-95ca-6bc1e6df99d8
md"We can see that speeding up $k_3$ results in a faster consumption of substrate, and that the dashed line (full system) and the solid lines (aproximation) become closer."
...
Now, taking the previous equation and using the conservation of mass $s(t)+p(t)+c(t)=s(0)$ allows us to find directy the dynamics of the formation of product. If we take the derivative of this relation we have $\dot{s}(t)+\dot{p}(t)+\dot{c}(t)=0$. Now if we asumme that $\dot{c}(t)=0$ because we are in conditions of Pseudo Steady state, we obtain directly that $\dot{s}(t)=-\dot{p}(t)$, so:


```math
  \dot{p}  =  \frac{ k_3 \cdot k_1\cdot e_0\cdot s}{k_3+k_2+k_1 \cdot s} \tag{29}
```
##
Puting all as function of variable $p$, we have 

```math
  \dot{p}  =  \frac{ k_3 \cdot k_1\cdot e_0\cdot (s_0-p-c_{eq})}{k_3+k_2+k_1 \cdot (s_0-p-c_{eq})} \tag{29}
```
##
Now, this equation can be directly used to calculate the intial rate of the formation of product, assuming that at $t=0$, there is no complex and no product (therefore $c=0$ and $p$=0). This way, we arrive at the following reaction: 

```math
  \dot{p}  =  \frac{ k_3 \cdot e_0\cdot s_0}{\frac{k_3+k_2}{k_1}+s_0} \tag{31}
```
##
which predicts the initial turnover rate of the enzymatic reaction $V_0$ as a function of initial substrate concentration $s_0$. Rearranging terms:
...
Based on this approximation, we can reproduce the famous Michaelis-Menten plot, which is basically a curve that shows the dependence of the velocity of the reaction in terms of production of product (i.e., $\dot p$) on the concentration of substrate. 


"

# ╔═╡ cf7ec8e6-3293-49ed-805e-640edc940967
md"##
We can identify three regimes:
- __linear__: there is a lot of enzyme available to bind the substrate, so the velocity of teh reaction increases linearly with the concetration of subtrate, as it occurs in noncatalized reactions. 
...
md" 
##
> __Task 4__: (a) Plot the Michaelis-Menten curve ($v_0$ versus $e_0$) for the parameters used in the previous tasks.
> (b) Use the same values to generate the Michaelis-Menten curve ($v_0$ versus $e_0$) using the solution of the full system of equations.
> (c) Use the same values to generate the Michaelis-Menten curve ($v_0$ versus $e_0$) using the solution of the system after the Pseudo Steady state aproximartion.
> (d) Change values of of $s_0$ and $e_0$ where the aproximation gets closer to teh real solution. Do you have to increase or decrease $s_0$ and $e_0$ ? Justify your answer. 
"

# ╔═╡ 8f1a6377-1894-4c0b-8dfe-727c3d3c1c7e
Foldable("Click to reveal solution of Task 4:",md"
##

To compare the analytical equation resulting from the Michaelis-Menten approach with the speed of the reaction in the full non-approximated system, we have to solve the numerical system, and compare the rates of the reactions predicted by the Mass action and the Michaelis-Menten approach.")
...
begin
	...
	plot(s_vector,k3 * e0_ * s_vector ./(s_vector.+(k3+k2)/k1),label="Michaelis Menten",size = (800, 300),left_margin = [10px 0px], bottom_margin = 15px)
	title!("Michaelis-Menten constant = $((k3+k2)/k1)")
	xlabel!("Concentration of substrate")
	ylabel!("Initial Speed of the reaction")
end

# ╔═╡ feeb275b-f7f2-4d10-9449-48657dc3599a
#=╠═╡
begin
	prob10 = ODEProblem(enzyme_kinetics10!,[s0],(0.0,100.0),[k1,k2,k3,e0,0.0])
	sol10 = solve(prob10)
	p111=plot(sol10,size = (800, 300),left_margin = [10px 0px], bottom_margin = 15px)
	title!("Enzyme-substrate kinetics simplified using mass action")
	xlabel!("Time [s]")
	ylabel!("Concentration [M]")


	p_solution3=[(s0 - u[1]) for (u,t) in tuples(sol10)]
	plot!(sol10.t,p_solution3,
	        label="p calculated",        
	        linealpha = 0.5,
	        linewidth = 3,
	        linestyle= :dash,
	...
	c_solution3=[(k1 * e0 * u[1])/(k3+k2+k1 * u[1]) for (u,t) in tuples(sol10)]
	plot!(sol10.t,c_solution3,
	        label="c calculated",        
	        linealpha = 0.5,
	        linewidth = 3,
	        linestyle= :dash,
...
begin
	...
	p000=plot(s_vector,k3 * e0_ * s_vector ./(s_vector.+(k3+k2)/k1),label="Michaelis Menten",)
	for i in 1:100 
	...
	p000=plot!(s_vector,speed_vector,label="rate full mass action",size = (650, 300),left_margin = [10px 0px], bottom_margin = 15px)
...
# ╔═╡ 97e3784d-0506-4e83-bc7a-a90f0aed2258
Foldable("Click to reveal the plot corresponding to Task 4:",p000)
##
...
manifest_format = "2.0"
project_hash = "e422e266a665d3880a4f1565ae0db2792ec25ab0"
